        ...
        @foreach ($groups as $group)
            <x-filament::card>
                <h3 class="text-lg font-medium mb-4 flex items-center">
                    @svg($group['icon'], 'w-6 h-6 mr-2 text-custom-500')
                    ...
                    @foreach ($group['links'] as $link)
                        <a href="{{ $link['url'] }}"
                           class="flex items-center p-2 rounded-lg transition-colors duration-200 bg-gray-100/10 hover:bg-gray-100 dark:hover:bg-gray-700">
                            @svg($link['icon'], 'w-5 h-5 mr-3 text-' . $link['color'] . '-500')
                            <span
                                class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $link['title'] }}</span>
                        </a>
                    @endforeach
                </div>
            </x-filament::card>
