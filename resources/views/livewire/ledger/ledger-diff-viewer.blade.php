    ...
    @if ($hasChangedColumns)
        <div class="flex justify-end mb-2">
            <x-mary-toggle wire:model.live="showChanges" label="{{ __('ledger.show_diff') }}"
                hint="{{ __('ledger.show_diff_hint') }}" position="left" class="m-3" />
        </div>
    @endif
    ...
    @if ($displayData)
        ...
        @foreach ($displayData as $group)
            <div wire:key="group-{{ $group['group_name'] }}-{{ $loop->index }}"
                class="collapse collapse-plus bg-base-200 mb-4" @if (!($collapsedStates[$group['group_name']] ?? false)) open @endif>
                            ...
                            @foreach ($group['columns'] as $column)
                                @php
                                    ...
                                    if ($column['status'] === 'modified') {
                                        $rowClass .= ' bg-warning/20';
                                    }
                                    if ($column['status'] === 'added') {
                                        $rowClass .= ' bg-success/20';
                                    }
                                    if ($column['status'] === 'deleted') {
                                        $rowClass .= ' bg-error/20';
                                    }
                                    //                                }
                                @endphp
                                <tr class="{{ $rowClass }}" wire:key="column-{{ $column['id'] }}">
                                    ...
                                    @if ($showChanges)
                                        ...
                                        <td class="w-1/3 lg:w-3/8 break-words pt-2 align-top">
                                            ...
                                            @elseif(isset($column['type']) && $column['type'] == 'files')
