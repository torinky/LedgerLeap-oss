...
<div x-data="{
     ...
     x-init="
        ...
        $watch('errorCount', (newVal, oldVal) => {
            if (newVal > 0 && (oldVal === 0 || oldVal === undefined) && !userClosed) {
                open = true;
            }
            if (newVal === 0 && oldVal > 0) {
                // Issue #26: すべてのエラーが解消されたら自動的に非表示にし、トップへ
                open = false;
                userClosed = false;
    ...
    {{-- メインカード --}}
    <div class="card bg-base-100 border-2 border-error shadow-xl overflow-hidden shadow-error/20"
         x-show="open && errorCount > 0"
         x-cloak
