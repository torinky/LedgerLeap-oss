<div
            ...
            <div class="absolute  place-self-center top-4">
                <div class="indicator">
                    <div class="indicator-item indicator-top indicator-center w-32">
                        @php
                            $count = $folderRecord->ledger_defines_count;
                        @endphp
                        @if ($count > 0)
                    ...
                    <div class="flex justify-center items-center">
                        ...
                        {{-- 階層移動ボタン --}}
                        <button class="btn btn-ghost" wire:click.prevent="changeCurrentFolder({{ $folderRecord->id }})"
    ...
    @foreach ($ledgerDefineRecords as $dKey => $ledgerDefineRecord)
        <div class="cursor-pointer p-4 rounded-lg shadow-lg bg-accent hover:shadow-accent hover:opacity-100 {{ in_array($ledgerDefineRecord->id, $selectedLedgerDefineIds) ? 'opacity-90' : 'opacity-60' }}  min-w-36 relative grid"
