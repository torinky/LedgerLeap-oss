...
- For browser-side Livewire / Alpine regressions, distinguish component markup from bootstrap/runtime mismatch first; if the console reports repeated `Can't find variable: ...` for `x-data` bindings, verify a single Alpine runtime, the layout's `@livewireScriptConfig`, and the current Vite bundle before changing the component logic.
