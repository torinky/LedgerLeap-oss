...
- If a dismissed preview must be reopened without a full page reload, pair the dismiss key with a reset nonce and remount the banner root with a stable `wire:key` so Alpine state is rebuilt.
- For critical announcements, make sticky behavior explicit: force sticky on, hide the close button, and let the UI communicate that the banner is not dismissible in the normal way.
- When the preview is driven from a form, keep the dismiss key derived from the current form state so different field combinations can be previewed independently.
...
- [ ] Dismissed previews can be reset and remounted without a full page reload.
- [ ] Sticky surfaces preserve offset and z-index with sibling layouts.
- [ ] Critical banners force sticky and hide the close affordance.
...
- Repo evidence: `docs/work/issue-drafts/2026-04-28_admin_announcement_banner_sprint3_2_completion.md`
- Repo evidence: `docs/work/issue-drafts/2026-04-28_admin_announcement_banner_sprint3_3_completion.md`
- Repo evidence: `docs/work/issue-drafts/2026-04-28_admin_announcement_banner_sprint3_4_completion.md`
...
- recheck_trigger:
  ...
  - a dismissed preview needs to reopen without a full reload in another feature