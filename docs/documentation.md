...
* **[添付ファイル機能](function/Attachment.md)**: VLM/OCR/Tika統合による高精度テキスト抽出、FileInspectorドロワーによる統合UI（Phase 1-5実装完了）。
...
*   **APIエンドポイント詳細**: 各APIエンドポイントの具体的なリクエストパラメータ、レスポンスフィールド、認証要件など。([API仕様概要](api/README.md)は作成済み)
*   （その他、必要に応じて追加）
...
*   [ColumnHtmlService](services/Ledger/ColumnHtmlService.md) - カラム値のHTML表示サービス（Phase 3リファクタリング完了）
