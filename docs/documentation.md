...
* **関連案件タブ** (Issue [#54](https://github.com/torinky/LedgerLeap/issues/54) / [#76](https://github.com/torinky/LedgerLeap/issues/76)):
...
開発環境（Laravel Sail）での基本操作コマンドです。セットアップ手順の詳細は [開発環境のセットアップ](development/environment-setup.md) を参照。
...
* **強力な全文検索とVLM/OCR統合**: 
    ...
    * **VLM統合**: **PaddleOCR-VL 0.9B**による高精度なビジュアル言語モデル。Markdown生成、構造化データ抽出、手書き文字認識に対応。
    ...
    * **FileInspectorドロワー**: 添付ファイルの詳細情報、処理状態、抽出テキスト（VLM/OCR/Tika）、履歴を統合表示するインタラクティブなUI。
* **非同期処理による堅牢なファイル解析**: 
...
*   [キューワーカと非同期処理](architecture/QueueProcessing.md)
...
*   [データベーススキーマ概要](database/schema.md)
...
*   [Gitブランチ戦略と開発フロー](development/branch_strategy.md) - 命名規則、日々の開発手順、コミット規約。新規参加者はまずここを読んでください。
    *   [Gitブランチ操作の詳細手順（Runbook）](runbooks/git-branch-workflow.md) - コマンドを含む完全な操作マニュアル
*   [テストのベストプラクティス](development/testing/README.md)（旧: [Testing-Best-Practices.md](development/Testing-Best-Practices.md)）
    * テスト実行は **Laravel Sail / Docker-based PhpStorm interpreter 必須**
*   [ユーティリティコマンド](development/utility-commands.md)
*   [デモ環境構築ガイド](development/demo-environment-setup.md) - サンプルデータ付きデモ環境の構築
*   [MCP アーキテクチャと動作フロー](development/MCP_Architecture_and_Flow.md) - LLM統合のための技術詳解
*   [MCP プロンプトガイドライン](development/MCP_Prompt_Guidelines.md) - LLM対話の最適化ガイド
...
- [ユーティリティコマンド一覧](development/utility-commands.md)
- [環境構築のセットアップガイド](development/environment-setup.md)

### デモ環境のセットアップ

LLMとの対話テストや機能デモのための環境構築については、以下の詳細ガイドを参照してください:
- **[デモ環境構築ガイド](development/demo-environment-setup.md)** - サンプルデータ付きデモ環境の構築手順
...
詳細は[開発環境のセットアップ](development/environment-setup.md)を参照してください。
...
* **VLM (Visual Language Model)**: 画像やPDFからテキストを抽出する高精度なAIモデル。LedgerLeapでは**PaddleOCR-VL 0.9B**を採用し、Markdown生成、構造化データ抽出、手書き文字認識が可能。
* **OCR (Optical Character Recognition)**: 光学文字認識。画像やスキャンされたPDFからテキストを抽出する技術。LedgerLeapでは**OcrMyPDF**を使用。
* **FileInspector**: 添付ファイルの詳細情報を表示するドロワーUI。VLM/OCR/Tikaの処理状態、抽出テキスト、履歴、権限を統合表示。共有用のQR導線はドロワー内にも用意され、表示中は先に閉じてから共有ダイアログを開く。

## 主な機能（詳細リンク）

* **[マイポータル](function/MyPortal.md)**: ユーザー個別のダッシュボード機能。
* **[台帳管理](function/Ledger.md)**: 台帳データの登録、編集、削除が可能。
* **[添付ファイル機能](function/Attachment.md)**: VLM/OCR/Tika統合による高精度テキスト抽出、FileInspectorドロワーによる統合UI。
* **[スコアリングシステム](features/scoring-system.md)**: ハイブリッド型情報価値評価により、重要な台帳を自動的に優先表示。

## 運用ガイド

* **[FileInspectorパフォーマンス測定](operations/fileinspector-performance-monitoring.md)**: 添付ファイルインスペクターのパフォーマンス測定機能の設定と使用方法。
* **[台帳レコード性能監視](operations/ledger-records-performance-monitoring.md)**: 台帳一覧の常時モニタ指標、回帰検知の閾値、warning ログの確認方法。
* **[データベースパフォーマンス監視](operations/database-performance-monitoring.md)**: データベースのパフォーマンス監視と最適化。
* **[モデル切り替えガイド](operations/model-switching-guide.md)**: VLMモデルの切り替え手順。
* **[アクセスとアクティビティ](function/AccessAndActivity.md)**: 詳細な権限表示と活動履歴の追跡機能。
* **[通知](function/Notification.md)**: 台帳データやフォルダ更新時にユーザーへ通知。権限や設定で制御。
* **[全文検索](function/Search.md)**: フォルダ階層以下やタグを対象に全文検索。**添付ファイル検索**も可能。
* **[類義語を使った処理](services/SynonymService.md)**: 類義語検索に対応。
* **[ワークフロー機能](function/WorkFlow.md)**: 台帳レコードに対する承認フロー機能。
* **[自動リンク機能](function/AutoLink.md)**: 特定の文字列を関連情報へ自動的にリンクする機能。
* **[テストコード](function/Test.md)**: テストコードの実装状況。
...
* `/docs`: ドキュメント（このファイル含む） **← ここに注目**
    * **公式ドキュメント（実装済み機能の仕様）:**
        ...
        * `/models/`: モデル仕様
        * `/operations/`: 運用ガイド
        * `/presentations/`: 公開プレゼンテーション、スライド、ポスター
        * `/services/`: サービス仕様
    * **作業ファイル（計画・設計・作業ログ）:**
        * `/work/`: 実装計画、設計書、作業記録
...
LedgerLeap の公開ドキュメントは、実装済み機能の技術仕様・運用ガイド・利用手順を reader-facing に整理したものです。

- 現在のシステム状態を反映する正式な仕様書として扱います
- 開発者・運用者・連携利用者が参照する公開リファレンスです
- 必要な追跡情報は companion issue / packet record 側で管理し、公開本文には内部 planning metadata を残しません
...
ブランチの命名規則や開発フローは [Gitブランチ戦略と開発フロー](development/branch_strategy.md) に従ってください。
...
*   [AttachedFile](models/AttachedFile.md)
*   [AutoLink](models/AutoLink.md)
*   [CustomActivity](models/CustomActivity.md)
*   [Folder](models/Folder.md)
*   [Ledger](models/Ledger.md)
*   [LedgerDefine](models/LedgerDefine.md)
...
*   [AutoLinkService](services/AutoLinkService.md)
*   [ColumnHtmlService](services/Ledger/ColumnHtmlService.md) - カラム値のHTML表示サービス
*   [LedgerService](services/LedgerService.md)
*   [NotificationService](services/NotificationService.md)
*   [SynonymService](services/SynonymService.md)
*   [UserService](services/UserService.md)
*   [WorkflowService](services/WorkflowService.md)
*   [ScoringServices](development/scoring-system.md) - スコアリングシステムの開発者ガイド
*   （整備中）
