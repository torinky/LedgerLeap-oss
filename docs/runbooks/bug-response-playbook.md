...
### 入口
- バグ / CI 失敗 / 回帰を起票する場合は `/.github/ISSUE_TEMPLATE/bug_report.yml` を使う
- 改善 / 機能追加 / 調査 / ドキュメント / リファクタの通常イシューは `/.github/ISSUE_TEMPLATE/issue_request.yml` を使う
...
- どの層の問題かを 1 文で固定する（例: app / Filament / vendor / translation / CSS / test）
- その層の所有者を確認する（自前コードか、vendor か、設定か、翻訳か）
- まず被害拡大防止が必要か
- ローカル問題か、tenant 固有か、全体影響か
- 実装修正に進んでよいか、先に調査を広げるべきか
- CI / 権限 / 検索 / Livewire / 外部サービス など、どの系統の問題か

不明な場合は、仮説を進める前に「不明」として明示し、追加確認を優先する。
...
- `storage/framework/testing/disks/...` や `storage/app/public` の runtime storage subtree が root 所有になっていないか
...
ここでは「最初の仮説が外れたときに何を見るか」を先に決める。早い段階で 1 つに決め打ちしない。

- 誤っていた仮説や失敗した切り分けも残す。
- うまくいかなかった試行は、次回なぜ不要かまで書く。
...
- UI / Livewire / Filament の見た目変更 → browser smoke を必須にする
- Tailwind utility を追加 → build を必須にする
- ACL / tenant / cache 関連 → cache 反映確認を必須にする
- 再現可能なロジック不具合 → 関連テストを優先する
...
- runtime storage subtree の exact-path 修正手順が再利用可能なら `.github/skills/sail-dev-workflow/SKILL.md` に同期する
