...
LedgerLeap のデータベース性能を継続的に監視するための手順とアラート基準を定義します。運用担当者が定期的に確認し、性能劣化を早期に検知することを目的とします。

## 対象範囲

- MySQL の主要パフォーマンスメトリクス監視
- スロークエリの検出と分析
- テナント別データ量の追跡
- Mroonga 全文検索の性能監視
- 監視の自動化（Artisan コマンド、Laravel Pulse）

### 対象外

- インフラレベルの監視（CPU、メモリ、ディスク）→ インフラ担当
- アプリケーションログ監視 → [活動履歴](../function/AccessAndActivity.md)
...
- パーティショニング設定: `database/migrations/2025_08_30_063606_add_tenant_id_to_tenant_tables.php`
- MySQL 設定: `docker/mroonga/my.cnf/mroonga.cnf`
- コンテナ設定: `docker-compose.yml`
