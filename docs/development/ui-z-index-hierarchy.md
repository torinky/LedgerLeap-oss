...
| **55** | 秘密区分スタンプ（`confidentiality-stamp`） | `components/ledger/confidentiality-stamp.blade.php` | navbar dropdown（30）より上。sticky announcement（50）と垂直方向に重なるため、クリック可能にするために z-[55] を設定。本実装時はアラート高さを考慮した動的位置調整を検討 |
...
| **40** | 検索セクション（sticky header） | `livewire/ledger/index-manager.blade.php` (`sticky top-16 z-40`) | 検索入力欯を含む sticky ヘッダー。内部の dropdown も含め、情報ブロック (`z-10`) やページネーションフッター (`z-20`) より前面に表示 |
| **40** | 検索サジェスト dropdown | `components/ledger/search.blade.php` (`relative z-40`, dropdown `z-40`) | 入力欄に紐づく popover。親ラッパーに `z-40` を持たせ、`records-table` の `z-20` fixed footer 等より前面に表示 |
...
## 修正履歴
- 2026-06-13: 検索サジェスト dropdown (`components/ledger/search.blade.php`) を `z-40` 階層に追加。親ラッパーにも `z-40` を設定し、`records-table` の `z-20` fixed footer 等より前面に表示。
