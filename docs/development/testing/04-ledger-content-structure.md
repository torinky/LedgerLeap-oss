...
**問題:** テストで `Ledger::factory()->create()` を直接呼ぶ場合、`normalizeByColumnDefine()` が呼ばれないため、
データ構造が実際のアプリケーションと異なる可能性がある。

```php
// ❌ 間違ったテストデータ作成（Phase 2 で発見）
...
// → DB には ["テストタイトル", ["タグ1", "タグ2"]] として保存される
