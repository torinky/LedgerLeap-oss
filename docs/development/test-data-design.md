...
- **追加の知見 (2026-01-31)**: `content` 配列のトップレベルキーは **必ず整数型 (int)** である必要があります。テストデータ作成時に `'0' => 'val'` のような文字列キーを使用すると、Retrieve時に不整合が発生し、`AutoLinkService` 等の連動機能が正常に動作しない原因となります。
- **効果**: この問題は`AsColumnArrayJson`レベルで一元的に対処されたため、今後のSeederやテスト実装で開発者がこの問題を意識する必要がなくなった。ただし、配列インデックスの型（int）については現在も明示的な注意が必要です。
