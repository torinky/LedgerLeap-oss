...
テナントに属する全テーブルにおいて、`tenant_id` が確実に設定されていることは、システムの安全性と直結します。

### 7.1. tenant_id 消失の防止

以下のケースでは、`tenant_id` が意図せず `null` になるリスクがあるため、特に注意してください。

- **手動でのレコード作成**: `Model::create([...])` などで属性を直接指定する場合、`tenant_id` を含め忘れると、グローバルアクセス可能な（あるいは誰からも見えない）浮いたデータになります。
- **バックグラウンド処理**: 下位レイヤー（Service等）で直接 DB 操作を行う場合。

### 7.2. データ不整合の自動検知

本プロジェクトでは、定期的にデータ不整合をチェックするテスト (`tests/Feature/DatabaseIntegrityTest.php`) を運用しています。

- **チェック内容**: テナントテーブルであるにもかかわらず `tenant_id` が `null` になっているレコードの有無。
- **推奨**: 新しいテナントテーブルを追加した際は、必ずこのテストに対象テーブルを追加してください。
