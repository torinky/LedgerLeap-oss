...
> **開発フロー全体については [Git ブランチ戦略と開発フロー](./branch_strategy.md) を参照してください。** ブランチの命名規則、日々の開発手順、コミットメッセージ規約をまとめています。
...
*   **Z-index 管理**: UIコンポーネントの重なり順を制御するため、[UI Z-Index 階層管理](./ui-z-index-hierarchy.md) のガイドラインに従います。モーダルなどは `x-teleport` を使用して body 直下に配置することを推奨します。
...
## モデルオブザーバー & イベントリスナー (2026-02-08 追加)

### 1. 責務の分離
- **`booted()` 内のクロージャ**: シンプルな処理（例：関連モデルの微調整、単一のキャッシュクリア）に適しています。
- **Observers (オブザーバー)**: 複数のイベントにまたがる複雑なロジックや、依存関係が多い処理（サービス層の呼び出しなど）に適しています。

### 2. 広範囲に影響するキャッシュクリア
`Role` や `Organization` のように、変更が多数のユーザーに影響を与えるモデルでは、個別のユーザーをループしてキャッシュをクリアするのではなく、システム全体のキャッシュフラッシュ（`flushAllUserPermissionsCache` 等）を検討してください。
- **理由**: 大規模な組織においてループ処理はパフォーマンス低下（タイムアウト）の原因となります。

### 3. デバッグの明示性
Observer内でのログ出力は、システムの隠れた挙動を追跡するために重要です。重要なキャッシュ操作などには `Log::info` を含め、どのイベントが原因で実行されたかを明記してください。
