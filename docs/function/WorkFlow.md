...
### ステップ 13: ワークフロー最終承認条件の厳格化

* **目的:**
  フォルダに設定された全ての必須点検ロールおよび必須承認ロールによる処理が完了していなければ、ワークフローが最終承認 (
  `APPROVED`) 状態に移行できないようにシステム制御を完全に実装する。
* **方針:** `WorkflowService` の最終承認アクション (`approve` メソッド)
  実行時に、必須ロールの処理状況を最終チェックし、条件未達の場合はエラーメッセージを表示して処理を中断する。
* **タスク:**
    1. **`WorkflowService::approve` メソッドの最終チェック強化:**
        * メソッドの冒頭で、対象 `Ledger` の `canBeFinallyApproved()` (または `getRequiredRolesProgressDetails()`
          を直接利用) を呼び出し、全ての必須点検ロールと必須承認ロールの処理が完了していることを**最終確認**する。
        * もし未完了の必須ロールが存在する場合、例外をスローするか、エラーを示す明確な戻り値を返す（ステップ12で既に実装されているが、ここを最終関門とする）。
    2. **UIフィードバックの再確認 (`Show.php` / `ModifyColumn.php`):**
        * `approve` アクション実行時に `WorkflowService`
          からエラーが返された場合に、ユーザーに「未完了の必須ロールがあるため承認できません。([未完了ロール名リスト])
          」のような具体的なエラーメッセージが表示されることを確認。
        * 「承認」ボタンの表示/非活性化ロジックが、この最終条件を正しく反映していることを確認。
    3. **関連ドキュメント更新:** ワークフローの最終完了条件が変更されたことを明記。
* **成果物:**
    * フォルダごとに定義された必須の承認プロセスがシステム的に完全に担保されるワークフロー。
    * ユーザーが完了条件を満たしていない場合に、その理由が明確にフィードバックされるUI。
