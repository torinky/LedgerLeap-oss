...
### ステップ 11: ワークフロー進捗の高度な可視化と複数必須ロール達成状況の表示

*   **目的:** フォルダに設定された複数の「必須点検ロール」および「必須承認ロール」について、現在のワークフローでどのロールが処理を完了したかを判定・表示する。進捗状況を直感的に把握できるUI（アイコン、バッジ、インジケーター、ツールチップ）を提供し、ユーザーが次のアクションを理解しやすくする。
*   **方針:**
    ...
    *   **この段階では、必須ロールの処理完了をシステムが強制的な遷移条件とするのではなく、あくまで「可視化」と「目安の提示」に留める。ただし、承認済み (`APPROVED`) ステータスの場合でも、必須ロールが未完了であればその旨をUI上で明示する。**
*   **タスク:**
    1.  **DBスキーマ変更とモデル修正 (必須ロール設定):**
        *   **中間テーブル作成 (`folder_required_roles`):** `folder_id`, `role_id`, `type` (ENUM: `inspector`, `approver`) を持つ。複合ユニーク制約設定。
        *   **`Folder` モデル修正:** `requiredInspectorRoles()` と `requiredApproverRoles()` リレーション (Many to Many) を定義。
    2.  **設定UI (Filament `FolderResource` / Livewire フォルダ編集フォーム):**
        *   フォルダ作成/編集フォームで、複数の必須点検ロールと必須承認ロールを `Select` (multiple) や `CheckboxList` などで選択・保存できるようにする。中間テーブルへのレコード作成/削除を行う。
    3.  **必須ロール処理状況判定ロジックの実装 (`Ledger` モデル or `WorkflowService`):**
        *   `Ledger` モデルに `getRequiredRolesProgressDetails(): array` メソッドを追加。
        *   **比較基準Diffの特定:** 「最後に内容が変更された `LedgerDiff` (contentあり)」を特定するヘルパーメソッドを実装。
        *   **必須ロール処理状況判定:**
            *   フォルダに設定された必須点検/承認ロールを取得。
            *   基準Diff以降の `LedgerDiff` 履歴を参照。
            *   各必須ロールについて、そのロールを持つユーザーが `modifier_id` となり、適切なステータス変更を実行した最初の記録があるかを確認。
            *   結果を詳細な配列で返す（例: `['inspection' => ['total' => 2, 'completed_count' => 1, 'completed_roles' => [{id, name}], 'pending_roles' => [{id, name}]], 'approval' => [...]]`）。
    4.  **UIへの進捗表示実装:**
        *   **台帳詳細画面 (`Show.php` / `show.blade.php`):**
            *   点検と承認それぞれの進捗をメーター（プログレスバー）と数値（例: `1/3`)で表示。ツールチップで完了済み/未完了のロール名を表示。
            *   **もし `APPROVED` ステータスにも関わらず未完了の必須ロールがある場合は、警告アイコンやメッセージでその旨を明示する。**
        *   **タスクリスト (`PendingList.php` / `pending-list.blade.php`, `OtherRelatedTasksList.php` / `other-related-tasks-list.blade.php`):**
            *   現在のステータス表示の横（または近く）に、点検と承認の残り必須ロール数を小さなバッジやアイコンで簡潔に表示。ツールチップで詳細表示。
        *   **アクションボタンへのインジケーター表示 (任意・高度):**
            *   「点検依頼」「承認申請」ボタンの横に、次のステップで対応が必要な必須ロールが残っている場合、それを促すインジケーターを表示することを検討。
    5.  **通知への応用検討 (オプション):**
        *   必須ロールの処理が1つ完了するたび、または全ての必須ロールの処理が完了した際に、関係者に状況を通知する。
*   **成果物:**
    *   フォルダごとに複数の必須点検/承認ロールを設定できる機能。
    *   台帳詳細画面およびタスクリストで、これらの必須ロールの処理進捗が、アイコン、バッジ、メーター、ツールチップなどを活用して直感的に可視化されるUI。
    *   ユーザーがワークフローのボトルネックや次に誰の処理が必要かの目安を把握しやすくなる。
    *   承認済みでも必須ロールが未完了の場合、その状況がUI上で分かる状態。

---

### ステップ 12: (新規) ワークフロー完了条件の厳格化

*   **目的:** フォルダに設定された全ての必須点検ロールおよび必須承認ロールによる処理が完了していなければ、ワークフローが最終承認 (`APPROVED`) 状態に移行できないようにシステム制御を実装する。
*   **方針:** `WorkflowService` の最終承認アクション (`approve` メソッド) 実行前に、必須ロールの処理状況をチェックし、未完了の場合はエラーメッセージを表示して処理を中断する。
*   **タスク:**
    1.  **`WorkflowService::approve` メソッド修正:**
        *   メソッドの冒頭で、対象 `Ledger` に関連するフォルダの必須承認ロール（および必須点検ロール）の処理が全て完了しているかを確認するロジックを追加（ステップ11で実装した `Ledger::getRequiredRolesProgressDetails()` を利用）。
        *   もし未完了の必須ロールが存在する場合、例外をスローするか、エラーを示す特定の戻り値を返す。
    2.  **UIフィードバック (`Show.php` / `ModifyColumn.php`):**
        *   `approve` アクション実行時に、`WorkflowService` からエラーが返された場合、ユーザーに「未完了の必須ロールがあります。([ロール名リスト])」のような具体的なエラーメッセージをトーストやモーダルで表示する。
        *   「承認」ボタンを非活性化するか、クリック時に上記エラーを事前表示するUIも検討。
