...
*   **目的:** フォルダに設定された複数の「必須点検ロール」および「必須承認ロール」について、現在のワークフローでどのロールが処理を完了したかを判定・表示する。進捗状況を直感的に把握できるUI（アイコン、バッジ、インジケーター、ツールチップ）を提供し、ユーザーが次のアクションを理解しやすくする。担当者選択UIもこの複数必須ロール情報を活用するように改修する。
*   **方針:**
    ...
    *   必須ロールの処理完了は、この段階ではシステム的な強制条件ではなく、あくまで「可視化」と「目安の提示」に留める。ただし、承認済み (`APPROVED`) ステータスの場合でも、必須ロールが未完了であればその旨をUI上で明示する。
*   **実施済みタスク:**
    1.  **DBスキーマ変更とモデル修正 (複数必須ロール設定):**
        *   中間テーブル `folder_required_roles` (`folder_id`, `role_id`, `type`) を作成。[完了]
        *   `Folder` モデルに `requiredInspectorRoles()` と `requiredApproverRoles()` リレーション (Many to Many) を定義。[完了]
    2.  **設定UI修正 (Filament `FolderResource` / Livewire フォルダ編集フォーム):**
        *   フォルダ作成/編集フォームで、複数の必須点検ロールと複数の必須承認ロールを選択・保存できるようにUIを実装（`Select` multiple 等を使用）。中間テーブルへのレコード作成/削除を行う。[完了]
    3.  **必須ロール処理状況判定ロジックの実装 (`Ledger` モデル):**
        *   `Ledger` モデルに `getRequiredRolesProgressDetails(): array` メソッドを実装。[完了]
            *   比較基準Diff特定ヘルパー (`findBaselineDiffForProgressCheck`) を実装し、「最後に内容が変更された `LedgerDiff`」を特定。
    ...
    4.  **UIへの進捗表示実装:**
        *   **台帳詳細画面 (`Show.php` / `show.blade.php`):**
            *   `getRequiredRolesProgressDetails()` の結果に基づき、点検と承認それぞれの進捗をメーター（プログレスバー）と数値で表示。ツールチップで完了済み/未完了のロール名を表示。[完了]
            *   `APPROVED` ステータスで未完了の必須ロールがある場合に警告アイコン・メッセージを表示。[完了]
        *   **タスクリスト (`PendingList.php` / `pending-list.blade.php`, `OtherRelatedTasksList.php` / `other-related-tasks-list.blade.php`):**
...
* **タスク (新規: ステップ11.A - 担当者選択UIの高度化):**
    1.  **`WorkflowAssigneeSelect` コンポーネントのデータ取得ロジック強化:**
        *   `fetchInspectorOptions()` / `fetchApproverOptions()` メソッドを改修。
        *   **A) 「過去の承認ルート」からの候補提示:**
            *   現在の `Ledger` と同じ `LedgerDefine` を持つ過去の完了済み (`APPROVED`) `Ledger` の `LedgerDiff` 履歴から、典型的な点検者・承認者のシーケンス（または単に頻出担当者）を抽出するロジックを実装。
            *   抽出した過去の担当者を、担当者選択リスト内で「前回の担当者」や「よく使われる組み合わせ」などのセクションで提示。
        *   **B) フォルダ必須ロール担当者の明確化と優先表示:**
            *   対象フォルダに設定された必須点検/承認ロールを取得。
            *   そのロールに所属するユーザーをリストアップし、候補リストの最上位グループに「（必須ロール名）の担当者」として明確に表示（アイコンや色で区別）。
            *   必須ロール内で実績のあるユーザーをさらに優先表示。
        *   **C) 実績ベース推奨ユーザー (上記Bに含まれない場合):**
            *   フォルダ内の全ユーザーを対象に、過去の点検/承認実績が多い順にユーザーをリストアップ。
        *   **D) その他の権限保有ユーザー:**
            *   対象フォルダに `INSPECT` / `APPROVE` 権限を持つユーザー（上記A～Cに含まれない）。
        *   **E) 直近の担当者履歴。**
        *   **候補者の追加情報取得:** 各候補ユーザーについて、主所属組織名と、そのユーザーが持つ全有効ロール（ラベル）を取得。
        *   **リスト統合と整形:** 上記A～Eの情報を統合し、優先度、推奨理由タグ（複数可）、組織・ロール情報を付加して最終的なオプションリストを生成。
    2.  **`WorkflowAssigneeSelect` のビュー (`workflow-assignee-select.blade.php`) 修正:**
        *   MaryUI の `<x-mary-choices>` コンポーネント（または `select` の `option` スロット）を使用。
        *   各選択肢に「ユーザー名」「推奨理由タグ（複数可）」「所属組織名」「保有ロールリスト（簡潔表示）」を表示できるようにカスタムHTMLを組み込む (`@scope('item', $user)` や `option-sub-label` などを活用)。
        *   検索機能 (`searchable`, `search-function`) は維持。
    3.  **親コンポーネントからの呼び出し修正:**
        *   `WorkflowAssigneeModal` を開く際に、対象フォルダの必須ロール情報（ロールIDリストまたはRoleコレクション）を `WorkflowAssigneeSelect` に渡すようにする。

*   **タスク (ステップ11.4 - UIへの進捗表示実装 - 前回計画と同様):**
    1.  **UIへの進捗表示実装:**
        *   **台帳詳細画面 (`Show.php` / `show.blade.php`):** 進捗メーターとツールチップで表示。承認済みで必須ロール未完了の場合は警告表示。
        *   **タスクリスト (`PendingList.php`, `OtherRelatedTasksList.php`):** 各タスク行に進捗サマリーをバッジ/アイコンとツールチップで表示。

*   **成果物:**
    *   フォルダごとに複数の必須点検/承認ロールを設定できる機能。
    *   担当者選択UIが大幅に改善され、フォルダ必須ロール、過去の承認ルート、実績、ユーザーの所属組織・ロール情報が総合的に提示され、より文脈に沿った適切な担当者選択が可能になる。
    *   台帳詳細画面およびタスクリストで、必須ロールの処理進捗が直感的に可視化されるUI。

---

### ステップ 12: 必須ロール処理状況の LedgerDiff への累積記録と進行制御

*   **目的:** `LedgerDiff` に、現在の `Ledger` 内容に対して処理が完了した必須点検ロールID群と必須承認ロールID群をJSONカラムとして累積的に記録する。これに基づき、ワークフローアクションボタンの表示/活性状態を制御し、各ステップ進行時に適切なチェックを行う。
*   **方針:**
    ...
    *   `Ledger` モデルの `getRequiredRolesProgressDetails()` は、`latestDiff` のこれらのJSONカラムと、フォルダに設定された全必須ロールを比較して達成状況を判定する。
    ...
    *   最終承認の厳格な条件（全必須ロール完了）はステップ13で行う。
*   **タスク:**
    1.  **DBマイグレーション (`LedgerDiff` テーブル修正):**
        *   `ledger_diffs` テーブルに以下のJSONカラムを追加 (nullable):
            *   `completed_inspector_role_ids` (点検が完了した必須点検ロールIDの配列)
            *   `completed_approver_role_ids` (承認が完了した必須承認ロールIDの配列)
    2.  **`LedgerDiff` モデル修正:**
        *   上記カラムを `$fillable` と `$casts` (`array`) に追加。
    3.  **`WorkflowService` メソッド修正 (重要):**
        *   **`saveDraft(...)` / `saveEditedRecord(...)` (内容変更時):**
            *   新しい `LedgerDiff` を作成する際、`completed_inspector_role_ids` と `completed_approver_role_ids` を**空の配列 `[]` で初期化**する。
        *   **`requestInspection(...)` (DRAFTからの点検依頼):**
            *   `saveDraft` が呼ばれた後なので、新しい `LedgerDiff` が作成される。このDiffでは `completed_*_role_ids` は空のまま。
        *   **`requestApproval(..., ?string $comments = null)` (点検者が承認申請):**
            *   新しい `LedgerDiff` を作成。
            *   直前の `LedgerDiff` (点検依頼時のもの) から `completed_inspector_role_ids` と `completed_approver_role_ids` を**引き継ぐ**。
            *   実行者（点検者）が所属し、かつ現在のフォルダの必須点検ロールに含まれるロールIDを、`completed_inspector_role_ids` 配列に追加（重複しないように）。
            *   `status` を `PENDING_APPROVAL` に。
        *   **`approve(..., ?string $comments = null)` (承認者が承認):**
            *   新しい `LedgerDiff` を作成。
            *   直前の `LedgerDiff` から `completed_inspector_role_ids` と `completed_approver_role_ids` を**引き継ぐ**。
            *   実行者（承認者）が所属し、かつ現在のフォルダの必須承認ロールに含まれるロールIDを、`completed_approver_role_ids` 配列に追加（重複しないように）。
            *   `status` は、この時点ではまだ `PENDING_APPROVAL` のまま（ステップ13で最終判定して `APPROVED` にする）。
        *   **`returnToDraft(..., ?string $comments)` (差し戻し):**
            *   新しい `LedgerDiff` を作成。
            *   `completed_inspector_role_ids` と `completed_approver_role_ids` は、差し戻し前の状態（直前のDiff）を**引き継ぐ**。これは、差し戻し後に再度同じ担当者が処理を再開する際に、以前の達成状況を参照できるようにするため（ただし、内容変更があれば次のDiff作成時にリセットされる）。
            *   `status` を `DRAFT` に。
        *   **`claimTask(..., ?string $comments)` (引き継ぎ):**
            *   新しい `LedgerDiff` を作成。
            *   `completed_inspector_role_ids` と `completed_approver_role_ids` は、引き継ぎ前の状態（直前のDiff）を**引き継ぐ**。
            *   担当者情報のみ更新。
    4.  **`Ledger` モデルの `getRequiredRolesProgressDetails()` メソッド修正:**
        *   **基準Diffの特定は不要になる。**
        *   フォルダに設定された必須点検/承認ロールを取得。
        *   `$this->latestDiff` (最新の `LedgerDiff`) を参照。
        *   `latestDiff->completed_inspector_role_ids` とフォルダの必須点検ロールを比較し、点検進捗（完了数、未完了ロールリストなど）を計算。
        *   `latestDiff->completed_approver_role_ids` とフォルダの必須承認ロールを比較し、承認進捗を計算。
        *   結果を詳細な配列で返す。
    5.  **アクションボタン表示/活性制御ロジックの実装 (UI側):**
        *   **台帳編集/詳細画面 (`ModifyColumn.php`, `Show.php`)、タスクリスト (`PendingList.php`):**
            *   「承認申請（点検完了）」ボタン: 自分が点検担当者でステータスが `PENDING_INSPECTION` なら活性。
            *   「承認」ボタン: 自分が承認担当者でステータスが `PENDING_APPROVAL` なら活性。
                *   **「いずれかの必須点検ロールによる点検が完了しているか」のチェック:** `latestDiff->completed_inspector_role_ids` が空でないことを確認。
    6.  **`WorkflowService` の進行条件チェックの簡略化:**
        *   `requestApproval`: 実行者の正当性チェックは維持。
        *   `approve`: 実行者の正当性チェックに加え、`latestDiff->completed_inspector_role_ids` が空でない（＝何らかの点検が完了している）ことをチェック。
*   **成果物:**
    *   `LedgerDiff` に必須ロールの処理完了状況が累積的に記録される。
    *   `Ledger` モデルで、最新のDiffを見るだけで現在の内容に対する必須ロールの達成状況が判定可能になる。
    *   UI上で、次に何をすべきかのヒントがより正確に提示される。
    *   ワークフローの各ステップ進行が、より実態に即した条件で制御される。
...
*   **方針:** `WorkflowService` の最終承認アクション (`approve` メソッド) 実行時に、必須ロールの処理状況を最終チェックし、条件未達の場合はエラーメッセージを表示して処理を中断する。
*   **タスク:**
    1.  **`WorkflowService::approve` メソッドの最終チェック強化:**
        *   メソッドの冒頭で、対象 `Ledger` の `canBeFinallyApproved()` (または `getRequiredRolesProgressDetails()` を直接利用) を呼び出し、全ての必須点検ロールと必須承認ロールの処理が完了していることを**最終確認**する。
        *   もし未完了の必須ロールが存在する場合、例外をスローするか、エラーを示す明確な戻り値を返す（ステップ12で既に実装されているが、ここを最終関門とする）。
    2.  **UIフィードバックの再確認 (`Show.php` / `ModifyColumn.php`):**
        *   `approve` アクション実行時に `WorkflowService` からエラーが返された場合に、ユーザーに「未完了の必須ロールがあるため承認できません。([未完了ロール名リスト])」のような具体的なエラーメッセージが表示されることを確認。
