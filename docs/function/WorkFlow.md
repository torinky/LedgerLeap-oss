...
### ステップ 11: 複数必須ロール対応と進捗・達成状況の高度な可視化

* **目的:** フォルダに設定された**複数の**
  「必須点検ロール」および「必須承認ロール」について、現在のワークフローでどのロールが処理を完了したかを判定・表示する。進捗状況を直感的に把握できるUI（アイコン、バッジ、インジケーター、ツールチップ）を提供し、ユーザーが次のアクションを理解しやすくする。担当者選択UIもこの複数必須ロール情報を活用するように改修する。
* **方針:**
    * `Folder` に複数の必須点検ロールIDと必須承認ロールIDを中間テーブル (`folder_required_roles`) で関連付ける。
    * 「最後に内容が変更された `LedgerDiff` 以降のDiff履歴」を基準に、各必須ロールの処理状況を判定する。
    * 進捗状況は、タスクリスト、詳細画面、アクションボタンなどに、状況に応じてアイコン、バッジ、インジケーター、ツールチップを組み合わせて簡潔に表示する。
    * 担当者選択UIでは、設定された複数の必須ロールのメンバーを優先的に提示し、各候補者の所属組織・保有ロールも表示する。
    *
    この段階では、必須ロールの処理完了をシステムが強制的な遷移条件とするのではなく、あくまで「可視化」と「目安の提示」に留める。ただし、承認済み (
    `APPROVED`) ステータスの場合でも、必須ロールが未完了であればその旨をUI上で明示する。
* **タスク:**
    1. **DBスキーマ変更とモデル修正 (複数必須ロール設定):**
        * **中間テーブル作成 (`folder_required_roles`):** `id`, `folder_id` (FK to `folders.id`), `role_id` (FK to
          `roles.id`), `type` (ENUM: `inspector`, `approver`), `timestamps`。`folder_id`, `role_id`, `type` で複合ユニーク制約。
        * **`Folder` モデル修正:** `requiredInspectorRoles()` と `requiredApproverRoles()` リレーション (Many to
          Many、中間テーブル経由) を定義。
        * **(既存の単一必須ロールカラム `required_inspector_role_id`, `required_approver_role_id`
          は廃止または移行処理を検討)**
    2. **設定UI修正 (Filament `FolderResource` / Livewire フォルダ編集フォーム):**
        * フォルダ作成/編集フォームで、複数の必須点検ロールと複数の必須承認ロールを、それぞれ `Select` (multiple,
          `relationship` を使用) や `CheckboxList` などで選択・保存できるようにする。中間テーブルへのレコード作成/削除を行う。
    3. **必須ロール処理状況判定ロジックの実装 (`Ledger` モデル or `WorkflowService`):**
        * `Ledger` モデルに `getRequiredRolesProgressDetails(): array` メソッドを実装。
        * **比較基準Diffの特定:** 「最後に内容が変更された `LedgerDiff` (contentあり)」を特定。
        * **必須ロール処理状況判定:**
            * フォルダに設定された複数の必須点検/承認ロールを取得。
            * 基準Diff以降の `LedgerDiff` 履歴を参照。
            * 各必須ロールについて、そのロールを持つユーザーが `modifier_id` となり、適切なステータス変更を実行した記録があるかを確認。
            * 結果を詳細な配列で返す（例:
              `['inspection' => ['total_roles' => Collection<Role>, 'completed_roles' => Collection<Role>, 'pending_roles' => Collection<Role>, 'completed_count' => N, 'total_count' => M], 'approval' => [...]]`）。
    4. **担当者選択UIコンポーネント (`WorkflowAssigneeSelect`) の改修:**
        * 親から渡されるパラメータに、対象フォルダの必須点検/承認ロール（IDの配列またはRoleのコレクション）を追加。
        * `fetchInspectorOptions()` / `fetchApproverOptions()` メソッドを改修:
            * **A) フォルダ必須ロール担当者 (最優先):** 渡された必須ロール群に所属するユーザーをリストアップ。
            * **B) 実績ベース推奨ユーザー (必須ロール内優先):** 上記Aのユーザーの中で、過去の点検/承認実績が多い順に並び替え、またはタグ付け。
            * **C) その他の権限保有ユーザー:** 対象フォルダに `INSPECT` / `APPROVE` 権限を持つユーザー（上記A, Bに含まれない）。
            * **D) 直近の担当者履歴。**
            * **候補者の追加情報取得:** 各候補ユーザーの主所属組織名と全保有ロール（ラベル）を取得。
            * **リスト統合と整形:** 優先度、理由タグ、組織・ロール情報を付加して最終的なオプションリストを生成。
        * ビュー (`workflow-assignee-select.blade.php`) を修正し、MaryUI `Choices` を使ってユーザー名、理由タグ、所属組織、保有ロールを表示。
    5. **UIへの進捗表示実装:**
        * **台帳詳細画面 (`Show.php` / `show.blade.php`):**
            * 点検と承認それぞれの進捗をメーター（プログレスバー）と数値（例: `1/3`)で表示。ツールチップで完了済み/未完了のロール名を表示。
            * もし `APPROVED` ステータスで未完了の必須ロールがあれば警告表示。
        * **タスクリスト (`PendingList.php`, `OtherRelatedTasksList.php`):**
            * ステータス表示の横に、点検/承認の残り必須ロール数を小さなバッジやアイコンで表示。ツールチップで詳細表示。
        * **(任意) アクションボタンへのインジケーター表示。**
    6. **通知への応用検討 (オプション)。**
* **成果物:**
    * フォルダごとに複数の必須点検/承認ロールを設定できる機能。
    * 担当者選択時に、複数の必須ロール、実績、組織/ロール情報が考慮された、より適切な候補が提示されるUI。
    * 台帳詳細画面およびタスクリストで、必須ロールの処理進捗が直感的に可視化されるUI。
    * ユーザーがワークフローの残ステップやボトルネックを把握しやすくなる。

---

### ステップ 12: ワークフロー完了条件の厳格化

* **目的:**
  フォルダに設定された全ての必須点検ロールおよび必須承認ロールによる処理が完了していなければ、ワークフローが最終承認 (
  `APPROVED`) 状態に移行できないようにシステム制御を実装する。
* **方針:** `WorkflowService` の最終承認アクション (`approve` メソッド)
  実行前に、必須ロールの処理状況をチェックし、未完了の場合はエラーメッセージを表示して処理を中断する。
* **タスク:**
    1. **`WorkflowService::approve` メソッド修正:**
        * メソッド冒頭で、対象 `Ledger` の `getRequiredRolesProgressDetails()`
          を呼び出し、全ての必須承認ロール（および必須点検ロール）の処理が完了しているかを確認。
        * 未完了の必須ロールが存在する場合、例外をスローするか、エラーを示す特定の戻り値を返す。
    2. **UIフィードバック (`Show.php` / `ModifyColumn.php`):**
        * `approve` アクション実行時に、`WorkflowService`
          からエラーが返された場合、ユーザーに「未完了の必須ロールがあります。([ロール名リスト])
          」のような具体的なエラーメッセージをトーストやモーダルで表示する。
        * 「承認」ボタンを、全ての必須ロールが完了するまで非活性化する、またはクリック時に上記エラーを事前表示するUIも検討。
    3. **関連ドキュメント更新:** ワークフローの完了条件が変更されたことを明記。
* **成果物:**
    * フォルダごとに定義された必須の承認プロセスがシステム的に担保されるワークフロー。
