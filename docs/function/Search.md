...
台帳レコード詳細画面の「関連案件」タブは、Mroongaの全文検索（識別番号検索）と
RAGベクトル検索（意味検索）の2軸を組み合わせて関連レコードを探索します。

識別番号検索には `scopeSearchContext` 経由で Mroonga の `MATCH() AGAINST()` を使用します。
意味検索には `RagSearchService::searchLedgers()` を利用します。

詳細は [関連案件タブ機能](../features/related-ledgers.md) を参照してください。
