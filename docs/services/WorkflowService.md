...
`WorkflowService` は、台帳レコードのライフサイクルにおけるワークフロー（下書き、点検、承認、差し戻しなど）の全プロセスを管理します。台帳のステータス遷移、関連する変更履歴 (`LedgerDiff`) の作成、担当者の割り当てと更新、およびワークフローイベントに応じた通知のトリガーなど、複雑なワークフローロジックを一元的に処理し、台帳の整合性と承認プロセスを保証します。
