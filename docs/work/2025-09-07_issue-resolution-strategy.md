...
2.  **ダッシュボードUIの問題:**
    ...
    *   **補足:** この問題は、より根本的な「中央管理画面とテナント画面のコンテキストが不明確」というUI/UX上の課題に起因することが判明した。最終的な解決策として、共通ヘッダーに「設定」メニューを設ける方針が採択された。詳細は以下のドキュメントを参照。
    *   **関連資料:** [共通ヘッダー「設定」メニュー実装計画書](./2025-09-08_centralized-settings-menu-implementation-plan.md)
3.  **ナビゲーションUIの問題:**
