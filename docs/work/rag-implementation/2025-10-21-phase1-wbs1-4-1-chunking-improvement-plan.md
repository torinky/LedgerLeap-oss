...
-   **基本型 (`text`, `textarea`, `number`, `url`, `auto_number`):
    *   値をそのまま文字列として返す。
-   **`select`型:
    *   `$column->options` から `$value` に対応するラベルを探して返す。見つからなければ`$value`自体を返す。
-   **`chk` (チェックボックス) 型:
    *   `$value` 配列内で `true` になっているキーを基に、`$column->options` から対応するラベルをすべて抽出し、カンマ区切り (`、`) の文字列として返す。
-   **`files`型:
    *   `$value` 配列の各要素（`hashed_name => original_name`）から、`original_name` のみを抽出し、カンマ区切りの文字列として返す。
-   **上記以外:
    *   値が配列の場合はJSON文字列に変換して返す。
    *   それ以外は文字列にキャストして返す。
-   **共通処理:
    *   変換後の文字列が空またはnullの場合は `null` を返す。
...
2.  **台帳データを項目グループごとに階層的に整形:**
    *   `LedgerDefine`の`columns`定義を`group`プロパティでグループ化し、`order`プロパティでソートする。
    *   新しい`group`が現れるたびに、H2見出し (`## {group名}`) を挿入する。グループ名が空の場合は `## {{ __('ledger.form.group_default') }}` を使用する。
    *   各項目について、`extractValueAsText` を使って `content` からテキスト値を取得する。値が `null` の場合はその項目をスキップする。
...
- [ ] **Task 1: `ProcessLedgerForRagJob` の改修**
    - [ ] `extractValueAsText` ヘルパーメソッドを実装する。
    - [ ] `buildMarkdownFromLedger` メソッドを実装する (`group`, `display_level` を考慮)。
    - [ ] `handle` メソッドの処理フローを新しいロジックに更新する。
- [ ] **Task 2: `ledger_chunks` マイグレーションの修正**
    - [ ] `2025-10-17-phase1-hybrid-search-plan.md` に記載のスキーマ定義を上記2.2の通り修正する。
- [ ] **Task 3: 既存データの再インデックス** (運用手順として後述)
...
- [ ] **`ProcessLedgerForRagJobTest` (修正・拡充)**
    - [ ] **値変換テスト:** `extractValueAsText`のユニットテストを作成し、`chk`や`select`など各カラム型で期待通りのテキストが返ることを検証する。
    - [ ] **Markdown生成テスト:** `group`や`display_level`を含む複雑な`LedgerDefine`のテストデータから、期待通りの階層構造を持つMarkdownが生成されることを検証する。
    - [ ] **チャンク化テスト:** 生成されたMarkdownが正しくチャンクに分割され、DBに保存されることを検証する。
    - [ ] **エッジケースのテスト:** `content`が空、`content_attached`が空、両方が空、といった様々なケースでジョブがエラーなく完了することを確認する。
