...
- **エンベディング (Embedding):** 
  ...
  - **実行環境の要件:**
    - Docker Composeで管理される専用Pythonコンテナ内で実行
    - 外部API呼び出し不要（完全にオンプレミスで動作）
    - メモリ要件: モデルサイズ + 処理バッファ（約3-4GB推奨）
    - CPU推奨スペック: 4コア以上（並列処理によるスループット向上）
...
| **次元数** | 1024 | 768 |
| **CPU推論速度** | 要実測（目標: 2秒/チャンク以内） | 要実測（より高速を期待） |
| **ライセンス** | MIT | MIT |
| **推奨度** | **◎ (第一候補)** | ○ (代替案) |
  ...
  - **パフォーマンス最適化手法:**
    - ONNXランタイムによる推論高速化
    - バッチ処理（複数チャンクの同時エンベディング）
    - 量子化モデル（精度とのトレードオフで検討）
...
**ローカル実行・オフライン動作の保証 [CRITICAL]:**
- エンベディングモデルは初回起動時にHugging Face Hubからダウンロードされる
- 本番環境では事前にモデルをダウンロードし、Dockerボリュームに配置する
- ランタイム中は一切の外部通信が発生しない
- モデルファイルのバージョン管理と更新戦略を確立する必要あり

**リソース制約への対応:**
- メモリ: Pythonコンテナに4GB割り当て（システム全体では8GB以上推奨）
- CPU: 4コア以上推奨（2コアでも動作するが処理時間は2倍程度）
- ストレージ: モデルファイル（2-3GB）+ チャンクデータ（台帳数に依存）
...
- **Docker Compose:** 既存のSail環境にPythonコンテナを追加
...
└───────────┘      └─────────────────┘      └─────────────────┘
                           ...
                           ▼                    ▼
                   ┌──────────────┐    ┌──────────────────┐
                   │ Embedding    │    │ ledger_chunks    │
                   │ Service      │    │ Table            │
                   │ (Python)     │    │ + Mroonga Vector │
                   │ [NEW]        │    │ Index [NEW]      │
                   │              │    │                  │
                   │ - CPU Only   │    └──────────────────┘
                   │ - ONNX RT    │
...
**実行環境の特徴:**
- **完全ローカル:** 外部API不要、Docker Compose内で完結
- **CPU最適化:** GPU不要、ONNX Runtimeによる高速化
- **モデル選択:** bge-m3（高性能）/ e5-base（高速）を実測で選定
...
│         Docker Compose設定（Python環境の追加）
├─ Day 2-3: Observer + Job実装
├─ Day 4: EmbeddingService実装（Python連携）
│         - bge-m3/e5-baseのCPU性能ベンチマーク測定
│         - ONNX最適化の効果検証
│         - バッチ処理性能の測定
└─ Day 5: 性能評価とモデルの最終決定
          - メモリ使用量の測定（Docker環境での実測）
          - 並行処理時の安定性確認
...
**成功基準:**
- [ ] **エンベディング処理時間が、1チャンクあたり平均2秒以内であること（CPU環境）**
- [ ] **Docker環境でのメモリ消費が4GB以内に収まること**
- [ ] **外部APIやクラウドサービスへの通信が一切発生しないこと**
...
**Go/No-Go判断:**
- ✅ Go → Phase 2へ移行
- ⚠️ 条件付きGo → 代替モデル（e5-base）採用またはバッチサイズ調整でPhase 2へ
- ❌ No-Go → 以下の対応を検討
  - 量子化モデルの導入
  - より軽量なモデル（all-MiniLM-L6-v2等）の検討
  - RAG導入の延期と代替手段の検討
...
Week 4: 運用準備
├─ 運用ドキュメント作成
│  - トラブルシューティング手順（Pythonコンテナ再起動等）
│  - パフォーマンスチューニングガイド
│  - Docker環境のリソース要件定義
├─ ディザスタリカバリ手順整備
│  - ledger_chunksテーブルのバックアップ戦略
│  - 全台帳再チャンク化の手順
└─ 本番環境展開・段階的ロールアウト
```
...
- 全台帳のチャンク化完了率 > 95%
- RAG検索の平均レスポンス時間 < 1秒
- システム全体のCPU使用率増加 < 30%（エンベディング処理含む）
- Pythonコンテナの稼働率 > 99.5%
- ユーザー満足度調査での肯定的評価 > 70%
...
1. **技術的負債の最小化**
   - 既存システムとの高い親和性
   - 新規ミドルウェア導入なし（Pythonコンテナは既存のDocker Compose環境に追加するのみ）
   - 既存の開発・運用パターンを踏襲

2. **ローカル環境・GPU不要での実行**
   - Docker Composeで完全に管理可能
   - 外部APIやクラウドサービスへの依存なし
   - CPU最適化されたONNXランタイムによる高速化
   - 一般的なサーバースペック（4コアCPU、8GBメモリ）で動作
...
**システム要件（最小推奨スペック）:**
- CPU: 4コア以上（Intel/AMD x86_64、8コア推奨）
- メモリ: 8GB以上（システム全体、16GB推奨）
- ストレージ: SSD推奨（データベース・モデルファイル用に20GB以上の空き）
- OS: Docker/Docker Composeが動作する環境（Linux、macOS、Windows WSL2）

**次のステップ:**
1. 本ドキュメントのレビュー・承認取得
2. Phase 1 PoC計画の詳細化
3. Docker Compose環境へのPythonコンテナ追加
4. 開発環境での実装開始
