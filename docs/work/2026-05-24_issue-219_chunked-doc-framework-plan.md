...
したがって、#225 はさらに **#226〜#230** に分解して進捗管理するのが妥当である。
...
| **#230 / 2-A3A** | format / evidence / source comments | doc format profile matrix / evidence field matrix / PHPDoc minimum rule |
...
### #230 / Sprint 2-A3A: publication packet の format / evidence / PHPDoc source comment 補強

**目的:** pilot 前に doc type ごとの section 形と、source に残す PHPDoc minimum rule を major project / phpDocumentor 系の根拠で固定する。

**出力:**
- doc format profile matrix
- packet evidence field matrix
- source comment target selection rule
- PHPDoc minimum rule (`@param`, `@return`, `@throws`, `@api` 採否)
- #229 に引き渡す updated template / runbook / prompt / harness rule

**完了条件:**
- packet ごとに `doc_format_profile` と required sections を選べる
- evidence に root source / external source / freshness が残る
- pilot packet が source comment policy を再発明せず着手できる

### #229 / Sprint 2-A4: pilot packet 実行と comment sync 検証

**目的:** 2-A1〜2-A3 と 2-A3A の設計が本当に回るかを小さく実証する。

**対象候補 / 実績:**
- `docs/getting-started/portal-and-navigation.md` ✅ pilot 実施
- `docs/api/search-api.md` ✅ pilot 実施
- `docs/api/bootstrap-manifest-api.md` ⏭ #219 backlog へ引き継ぎ
- `docs/api/mcp-client-guide.md` ⏭ #219 backlog へ引き継ぎ
...
**実証結果:**
- `portal-and-navigation` で `comment_sync_policy = not_applicable` を確認
- `search-api` で `comment_sync_policy = required` を確認
- #219 backlog は `portal-and-navigation` → `search-api` → `bootstrap-manifest-api` → `mcp-client-guide` の順で凍結
...
- **#225** を umbrella
- 2-A1〜2-A4 に加えて、spec-vs-implementation diff 由来の 2-A3A も個別 issue に分ける
...
4. **2-A3A** で format / evidence / source comment policy を補強
5. **2-A4** で pilot 実証し、#219 backlog を凍結
