...
本機能の実装完了に伴い、関連するドキュメントを更新しました。

*   **機能仕様書 (`/docs/function/Ledger.md`)**: 台帳管理機能の説明に、複数行カラムがMarkdown入力に対応している旨と、利用可能な主な記法（太字、リスト、コードブロックなど）の例を追記しました。
*   **機能仕様書 (`/docs/function/AutoLink.md`)**: 自動リンク機能の適用対象に「台帳レコードの複数行入力カラム」が追加されたこと、および処理順序に関する注記を追記しました。
*   **プロジェクト概要 (`/docs/README.md`)**: 「LedgerLeap の特徴と機能」セクションに、Markdown記法をサポートする「表現力豊かなテキスト入力」の項目を追加し、自動リンク機能が台帳本文にも適用されることを反映しました。