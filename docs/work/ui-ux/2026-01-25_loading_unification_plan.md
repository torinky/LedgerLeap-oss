...
### フェーズ7: 統合リアクティブ・ナビゲーション (Planned: 2026-01-31〜)
- [ ] **CP-7.1**: `IndexManager` 親コンポーネントの導入（状態の集約）
- [ ] **RF-7.2**: `Folder/Tree` と `RecordsTable` の `#[Reactive]` 統合
- [ ] **RF-7.3**: 単一リクエスト化による通信ギャップの完全解消
- [ ] **詳細は別紙**: [台帳リスト画面のリアクティブ統合計画](2026-01-31_index-reactive-integration-plan.md)
