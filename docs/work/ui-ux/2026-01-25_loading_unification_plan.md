...
- [ ] **CP-1.3**: レイアウトへのグローバル・プログレスバー導入
  - `app.blade.php`, `appWithDrawer.blade.php` への `x-mary-loading` (progress bar) 設置。
- [ ] **CP-1.4**: UX 微調整用 CSS
...
- [ ] **RF-2.2: ポータル・ナビゲーション (注目ポイント)**
  - [ ] `MyPortal` に Tier 2 スケルトンカードを導入（統計・権限情報の遅延対策）。
  - [ ] `FolderTree` 遷移時に Tier 2 オーバーレイを表示し、操作を一時保護。
- [ ] **RF-2.3: 詳細画面周辺**
