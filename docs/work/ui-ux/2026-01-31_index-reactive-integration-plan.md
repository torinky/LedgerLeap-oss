...
- [ ] **RF-7.5**: 検索実行ロジックを、親からの変更を検知する `rendered` フックまたは `render` メソッド内での自動計算に整理
- [ ] **RF-7.6**: `RecordsTable` 自体は URL 同期を持たず、親から渡された値のみで動作するように変更
- **成果物**: 単独では動作せず、親の指揮下で動く高速なリスト表示コンポーネント
- **確認事項**: 
    - 親コンポーネントの状態（`displayLevel`など）を書き換えたとき、リストが単一リクエストで更新されるか。

### ブロック3: `Folder/Tree` のリアクティブ統合
- [ ] **RF-7.7**: `Folder/Tree` のプロパティ（`currentFolderId`, `selectedFolderIds`）を `#[Reactive]` 化
- [ ] **RF-7.8**: フォルダ選択時の処理を `$this->dispatch('...')` から親のメソッド呼び出し（`$this->setFolder()` 等）へ変更
- [ ] **RF-7.9**: `RecordsTable` が受信していた `#[On('currentFolderChangedByTree')]` イベントリスナーを削除
- **成果物**: ツリーの選択状態とリストのフィルタ条件が完全に同期した単一の動作ユニット
- **確認事項**: 
    - ツリーをクリックした際、ツリーの選択ハイライトと右側のリスト更新が同時に発生し、通信が1回に集約されているか。
