...
### ✅ ステップ 1: `NumberType`を`AutoNumberType`にリファクタリング (完了)
...
### ✅ ステップ 3: `NumberingService`の作成と採番ロジックの実装 (完了)
...
### ✅ ステップ 4: レコード作成画面への採番ロジック組み込み (完了)
...
### ✅ ステップ 5: 重複チェック用カスタムバリデーションの実装 (完了)
...
*   **作業内容:**
    ...
    3.  **`validate()` メソッド実装:**
        ...
        *   Mroongaの`match`関数とカラム指定検索(`*W{N}`)を利用して、重複の可能性があるレコードをデータベースから効率的に絞り込む。
        *   取得したレコード群に対して、PHP側で厳密な比較を行い、最終的な重複を判定する。
        *   `$ignoreLedgerId` があれば、そのIDを持つレコードを検索対象から除外。
        *   該当レコードが見つかった場合は、`fail('validation.unique')->translate()` を呼び出す。
...
### 6.1. `NumberingService` のユニットテスト

*   **目的:** `NumberingService::getNextNumber()` メソッドが、与えられた設定と既存データに基づいて、次の自動採番番号を正確に計算し、フォーマットすることを確認する。
*   **テストファイル:** `tests/Unit/Services/NumberingServiceTest.php` を新規作成。
*   **テスト環境:**
    *   `RefreshDatabase` トレイトを使用して、テストごとにクリーンなデータベース状態を確保する。
    *   テストに必要な`LedgerDefine`と`Ledger`のデータは、テストメソッド内で直接作成するか、専用のファクトリ/シーダーを呼び出して準備する。
*   **テストケース:**

    1.  **レコードが存在しない場合の初期採番:**
        *   **内容:** 該当する台帳定義に`auto_number`カラムのレコードが1件も存在しない場合、連番が`1`から始まり、正しくゼロ埋めされ、接頭辞・版記号が適用されることを確認する。
        *   **例:** `prefix="DOC-", digits=3, revision="-A"` -> `DOC-001-A`
    2.  **`unique`=false (版記号考慮) の場合の採番:**
        *   **内容:**
            *   同じ接頭辞と版記号を持つ既存レコードの最大連番に+1されることを確認する。
            *   異なる版記号のレコードは、採番に影響しないことを確認する。
        *   **例:**
            *   既存: `DOC-001-A`, `DOC-002-A` -> 次: `DOC-003-A`
            *   既存: `DOC-001-A`, `DOC-001-B` -> 次 (版記号-A): `DOC-002-A`
    3.  **`unique`=true (版記号無視) の場合の採番:**
        *   **内容:**
            *   版記号を無視し、同じ接頭辞を持つ既存レコードの最大連番に+1されることを確認する。
            *   版記号が異なる既存レコードも、採番に影響することを確認する。
        *   **例:**
            *   既存: `PROJ-001-X`, `PROJ-002-Y` -> 次: `PROJ-003` (版記号は設定による)
    4.  **桁数 (`digits`) のテスト:**
        *   **内容:** `digits`パラメータが正しくゼロ埋めに適用されることを確認する。
        *   **例:** `digits=4`, 次の連番が`10` -> `0010`
    5.  **接頭辞 (`prefix`) / 版記号 (`revision`) が空の場合のテスト:**
        *   **内容:** 接頭辞や版記号が空文字列の場合でも、正しく採番・フォーマットされることを確認する。
    6.  **既存レコードの連番が非連続の場合のテスト:**
        *   **内容:** 既存レコードの連番が`001`, `005`のように非連続の場合でも、最大値+1が正しく計算されることを確認する。
    7.  **`content`カラムに数値以外のデータが含まれる場合のテスト:**
        *   **内容:** `content`カラム内の対象外のデータや、自動採番のパターンに一致しないデータが、採番ロジックに影響を与えないことを確認する。

### 6.2. `UniqueAutoNumber` ルールのユニットテスト

*   **目的:** `UniqueAutoNumber::validate()` メソッドが、指定された「接頭辞 + 連番」の組み合わせの重複を正確に検出し、編集時の`ignoreLedgerId`を正しく考慮することを確認する。
*   **テストファイル:** `tests/Unit/Rules/UniqueAutoNumberTest.php` を新規作成。
*   **テスト環境:**
    *   `RefreshDatabase` トレイトを使用して、テストごとにクリーンなデータベース状態を確保する。
    *   テストに必要な`LedgerDefine`と`Ledger`のデータは、テストメソッド内で直接作成するか、専用のファクトリ/シーダーを呼び出して準備する。
*   **テストケース:**
    ...
    1.  **重複がない場合:**
        *   **内容:** データベースに同じ「接頭辞 + 連番」の組み合わせが存在しない場合、バリデーションが成功することを確認する。
    2.  **重複がある場合 (新規作成時):**
        *   **内容:** データベースに同じ「接頭辞 + 連番」の組み合わせが既に存在し、`ignoreLedgerId`が指定されていない（または`null`）場合、バリデーションが失敗することを確認する。
    3.  **重複がある場合 (編集時 - 自身のレコード):**
        *   **内容:** 編集中のレコードのIDを`ignoreLedgerId`として渡し、そのレコード自身の「接頭辞 + 連番」が他のレコードと重複していない場合、バリデーションが成功することを確認する。
    4.  **重複がある場合 (編集時 - 他のレコード):**
        *   **内容:** 編集中のレコードのIDを`ignoreLedgerId`として渡したが、データベース内の**他のレコード**に同じ「接頭辞 + 連番」の組み合わせが存在する場合、バリデーションが失敗することを確認する。
    5.  **入力値が自動採番パターンに一致しない場合:**
        *   **内容:** 入力値が自動採番のパターン（例: `接頭辞 + 数字`）に一致しない場合、このルールがバリデーションを失敗させないこと（他のバリデーションルールに任せる）を確認する。
    6.  **空値のテスト:**
        *   **内容:** `null`や空文字列が渡された場合、このルールがバリデーションを失敗させないこと（`required`ルールに任せる）を確認する。
