...
| # | ドキュメントパス | 更新種別 | 優先度 | 推定工数 |
|---|-----------------|---------|--------|---------|
| 1 | `docs/function/Attachment.md` | 大規模更新 | 🔴 最高 | 3-4h |
| 2 | `docs/architecture/vlm-ocr-technology-selection.md` | 部分更新 | 🔴 高 | 1.5-2h |
| 3 | `docs/architecture/QueueProcessing.md` | 部分更新 | 🔴 高 | 1-1.5h |
| 4 | `docs/services/Ledger/ColumnHtmlService.md` | 部分更新 | 🟡 中 | 0.5-1h |
| 5 | `docs/README.md` | 小規模更新 | 🟡 中 | 0.5h |
| 6 | `docs/development/vlm-ocr.md` | 部分更新 | 🟡 中 | 1h |
| 7 | `docs/database/schema.md` | 新規作成 | 🟢 低 | 2-3h |
| 8 | `docs/models/AttachedFile.md` | 新規作成 | 🟢 低 | 1-2h |
| 9 | `docs/architecture/file-processing-flow.md` | 新規作成 | 🟡 中 | 2-3h |
| 10 | `.github/copilot-instructions.md` | 小規模更新 | 🟡 中 | 0.5h |
| 11 | `docs/development/performance-optimization.md` | 新規作成 | 🟡 中 | 2-3h |
| 12 | `docs/operations/fileinspector-performance-monitoring.md` | 部分更新 | 🟡 中 | 1h |
...
|-----------|---------|------------------|
| **3.2 UIコンポーネント** | 新規追加 | - |
| 3.2.1 FileInspectorドロワー | 概要、4タブ構成、開閉方法 | phase4-6_completion_summary.md |
| 3.2.2 Contentタブ | VLM/OCR/Tikaソース切り替え、検索機能、信頼度表示 | content_tab_ui_refinement.md |
| 3.2.3 Detailsタブ | ファイル情報、ベンチマーク、メタデータ | phase4_detailed_plan.md (WBS 4.3) |
| 3.2.4 Historyタブ | タイムライン、処理履歴、エラーログ | phase4_detailed_plan.md (WBS 4.4) |
| 3.2.5 Permissionsタブ | 権限バッジ、再処理アクション | phase4_detailed_plan.md (WBS 4.5) |
| **3.3 処理フロー図** | VLM→OCR→Tika統合フロー図追加 | phase3_detailed_plan.md |
| **3.4 処理状態管理** | 24パターンの状態遷移表 | phase4-6-4_ui_verification_checklist.md |
| **3.5 エンジン統合** | VLM/OCR/Tikaの役割と優先順位 | phase5_feature_comparison_report.md |
| **4. データモデル** | 新カラム追加の説明 | phase2_model_extension_plan.md |
| 4.1 attached_filesテーブル拡張 | vlm_processed_at, confidence等 | - |
| **5. 非同期処理** | ジョブフロー図更新 | phase3_detailed_plan.md |
| **6. パフォーマンス** | キャッシング戦略、測定結果 | phase4-6-5_performance_report.md |

**追加すべき図表:**
- FileInspectorドロワーのスクリーンショット（4タブ）
- VLM→OCR→Tika処理フローMermaid図
- 処理状態遷移図
- attached_filesテーブルER図（更新版）
...
|-----------|---------|------------------|
| **2. 実装状況** | セクション新規追加 | - |
| 2.1 Phase 1-5完了サマリー | 実装完了内容の要約 | phase4-6_completion_summary.md |
| 2.2 PaddleOCR-VL採用実績 | 選定理由、実装結果、精度評価 | attachment-ui-improvement-plan.md |
| 2.3 VLM統合アーキテクチャ | 実装済みアーキテクチャ図 | phase3_detailed_plan.md |
| **3. 技術評価（更新）** | 実測ベンチマーク追加 | phase4-6-5_performance_report.md |
| **4. 今後の展望** | Phase 6以降の改善計画 | phase5_detailed_plan.md |
...
| セクション | 更新内容 | 参照元ドキュメント |
|-----------|---------|------------------|
| **2.2 添付ファイル処理ジョブ** | VLM統合ジョブ追加 | phase3_detailed_plan.md |
| 2.2.1 ProcessVlmForAttachedFile | 新規追加 | - |
| 2.2.2 ProcessOcrForAttachedFile | 既存ジョブ説明更新 | - |
| 2.2.3 ProcessTikaForAttachedFile | 既存ジョブ説明更新 | - |
| 2.2.4 FinalizeAttachedFileProcessing | 新規追加（最終化ジョブ） | phase2_model_extension_plan.md |
| **2.3 ジョブチェーン** | VLM→OCR→Tika→Finalize順次実行 | phase3_detailed_plan.md |
| **2.4 エラーハンドリング** | リトライ戦略、フォールバック | phase3_detailed_plan.md |
| **3. ジョブフロー図** | Mermaid図更新 | - |
...
**目的:** WBS 5.2で得られたパフォーマンス最適化の知見を体系化
