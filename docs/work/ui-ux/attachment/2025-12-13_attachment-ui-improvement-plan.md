...
**最終更新:** 2025年12月15日（データ構造整理を反映）

総見積工数: **7日 (56h)** ← Phase 1完了後、データ構造整理を踏まえて再見積
...
| | **2.3** | **OCR後PDFダウンロード機能（ルート・コントローラー）** | - | **2h** | **-** | ⚠️ 要確認 |
| | **2.4** | **モデル拡張のテスト実装（Unit Test）** | - | **2h** | **2.1-2.3** | ⚠️ 新規 |
| **3. 基盤改修** | 3.1 | `ColumnHtmlService` のリファクタリング (Bladeコンポーネント化) | - | 4h | 2.4 | 依存更新 |
| | 3.2 | リスト表示用Bladeコンポーネントの実装 (`x-ledger.attachment-list`) | - | 4h | 3.1 | - |
| | 3.3 | 一覧画面 (`RecordsTable`) での簡易表示モード実装 | - | 2h | 3.2 | - |
| **4. ドロワー実装** | 4.1 | `FileInspector` Livewireコンポーネントの作成（基本構造） | - | 3h | 2.4 | 依存更新 |
...
| | 5.6 | 多言語対応 (ja.json への翻訳キー追加) | - | 1h | 5.1-5.5 | - |
