...
総見積工数: **5.5日 (44h)** (モックアップ・セキュリティ・パフォーマンス工程を追加)
...
| | 2.3 | 一覧画面 (`RecordsTable`) での簡易表示モード実装 | - | 2h | 2.2 |
| **3. ドロワー実装** | 3.1 | `FileInspector` Livewireコンポーネントの作成 | - | 4h | - |
| | 3.2 | ドロワーUIの基本実装 (`x-drawer`, タブ, レスポンシブ) | - | 5h | 3.1 |
| | 3.3 | 親コンポーネント (`Show`, `ModifyColumn`) とのイベント連携 | - | 2h | 2.2, 3.1 |
| | 3.4 | アクセス権限チェックの実装（ポリシー連携） | - | 2h | 3.1 |
| **4. 情報拡張** | 4.1 | `AttachedFile` モデルへのヘルパーメソッド追加 (履歴・ログ取得) | - | 3h | - |
| | 4.2 | ドロワー内コンテンツの実装 (基本情報, テキスト, 履歴) | - | 6h | 3.2, 4.1 |
| | 4.3 | 多言語対応 (ja.json への翻訳キー追加) | - | 1h | 4.2 |
| **5. パフォーマンス最適化** | 5.1 | Eager Loading戦略の実装 (`with()` 句の最適化) | - | 2h | 4.1 |
| | 5.2 | キャッシング機構の検討・実装（大量ファイル対策） | - | 2h | 5.1 |
| **6. 仕上げ** | 6.1 | RPA互換性検証 (ダイレクトリンクの確認) | - | 1h | 2.2 |
| | 6.2 | Feature/Livewireテスト実装 (権限・イベント・表示) | - | 3h | ALL |
| | 6.3 | モバイル・タブレット実機検証 | - | 2h | 3.2 |
| | 6.4 | Pint実行・コミット準備 | - | 0.5h | ALL |
