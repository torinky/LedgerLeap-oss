...
**作成日:** 2025年12月13日
**最終更新:** 2025年12月19日（Phase3詳細計画更新: MIMEタイプ拡張、時間/容量表示改善、ダウンロードリンク整理、MimeTypeHelper追加）  
**ステータス:** ✅ Phase 1完了 → ✅ Phase 2完了 → 📋 Phase 3計画更新
**対象:** LedgerLeap開発チーム
...
総見積工数: **9.25日 (74h)** ← Phase 3詳細化により+6h（MimeTypeHelper+2h, MIMEタイプ拡張+1h, 時間/容量表示+1h, ダウンロードリンク整理+1h, UIモックアップ検証強化+1h）
...
| **3. 基盤改修** | **3.0** | **`MimeTypeHelper` クラスの実装** | - | **2h** | **2.3** | 📋 詳細計画完了 |
| | **3.1** | **`x-ledger.attachment-list` コンポーネントの強化** | - | **6h** | **3.0** | icon-only + MIMEタイプ拡張 + 時間/容量表示 |
| | **3.2** | **`ColumnHtmlService` のリファクタリング** | - | **6h** | **3.1** | Bladeコンポーネント化 + ダウンロードリンク整理 |
