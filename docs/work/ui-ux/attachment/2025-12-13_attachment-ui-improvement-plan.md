...
**作成日:** 2025年12月13日
**最終更新:** 2025年12月20日（Phase 4精査完了、計画拡充）  
**ステータス:** ✅ Phase 1完了 → ✅ Phase 2完了 → ✅ **Phase 3完了** → 📋 **Phase 4精査完了・実装準備完了**
**対象:** LedgerLeap開発チーム
...
- **[Phase 4 詳細計画: インスペクター実装](/docs/work/ui-ux/attachment/2025-12-20_phase4_detailed_plan.md)** 📋 **精査完了**
...
**主要な実装内容:**
- [ ] **4.2.1**: `previewable_text` アクセサをバインド、最終化前は「処理中」表示
- [ ] **4.2.2**: VLM固有フィールドのバインドとMarkdownレンダリング
- [ ] **4.2.3**: **UI分岐実装**（VLM優先 → OCR → Tika、信頼度バッジ表示）
- [ ] **4.2.4**: **エラー時のフォールバック**（全処理失敗時の表示と再処理ボタン）
- [ ] **4.2.5**: Alpine.jsクリップボードコピー機能（既存VLMモーダルから移植）
- [ ] **4.2.6**: VLM Markdown/JSONダウンロードボタン（`files.download-vlm` ルート使用）
- [ ] **4.2.7**: `showVlmPreviewEvent` 完全置き換え確認

**データ整合性の懸念:**
- ⚠️ **content_attachedキー構造**: Phase 5/6で `image.jpg` → `image.pdf` に変更
- 対策: `previewable_text` アクセサの堅牢性を検証、キー不一致時のフォールバック実装
...
**主要な実装内容:**
- [ ] **4.5.1**: 権限情報表示（`LedgerPolicy` 経由で台帳権限を表示）
- [ ] **4.5.2**: **ファイル削除アクション**（確認ダイアログ付き、Soft Delete使用）
- [ ] **4.5.3**: **再処理アクション**（一般ユーザーとAdmin権限の分岐）
- [ ] **4.5.4**: **VLM再処理**（管理者専用、低信頼度時の再処理）
- [ ] **4.5.5**: **権限チェック**（各アクションボタンに `@can` ディレクティブ）
- [ ] **4.5.6**: アクションタブまたはセクションの追加
...
**最終更新:** 2025年12月21日（Phase 4精査により工数修正: 30h→38h）

総見積工数: **12.0日 (96h)** ← Phase 4精査により+8h（権限チェック+2h, UI分岐検討+2h, エラーハンドリング+1h, 4.2内容タブ拡充+2h, 詳細タブ拡充+1h）
...
| | **4.2** | **内容（Content）タブ (VLM/OCR統合)** | - | **9h** | **4.1** | 📋 **詳細化済（Markdown/ハイライト/切替含む）** |
| | **4.3** | **詳細（Details）タブ** | - | **4h** | **4.1** | 📋 **詳細化済（プレビュー・処理時間含む）** |
| | **4.4** | **履歴（History）タブ (タイムライン)** | - | **5h** | **4.1, 2.2** | 📋 **詳細化済（エラーログ・フィルタ含む）** |
| | **4.5** | **権限とアクション（Actions）タブ** | - | **6h** | **4.1** | 📋 **詳細化済（VLM再処理含む）** |
