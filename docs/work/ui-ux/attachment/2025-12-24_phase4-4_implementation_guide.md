...
|---------|---------|------|--------|---------|------|
| **4.4.0** | **LogsActivityトレイト追加（オプショナル）** | **0.5h** | **低** | なし | ⏭️ スキップ |
| **4.4.1** | **ActivityLogFormatter拡張と翻訳追加** | **1h** | **必須** | なし | ✅ 完了 |
| **4.4.2** | **タイムラインデータ生成ロジック実装** | **2h** | **必須** | 4.4.1 | ✅ 完了 |
| **4.4.3** | **履歴タブUI実装** | **1.5h** | **必須** | 4.4.2 | ✅ 完了 |
| **4.4.4** | **動作検証とテスト** | **0.5h** | **必須** | 4.4.3 | ✅ 完了 |
...
#### チェックリスト
- [x] `ActivityLogFormatter.php` に `AttachedFile` の分岐追加
- [x] ダウンロードイベントの分岐追加
- [x] `lang/ja/ledger.php` に翻訳キー追加（6キー: 4+2追加）
- [x] `./vendor/bin/sail pint` でコード整形
- [x] 既存のActivityHistoryDisplayでエラーが出ないか確認
...
#### チェックリスト
- [x] `getSystemTimelineAttribute()` 実装（7イベント対応）
- [x] `getUserTimelineAttribute()` 実装（50件制限）
- [x] 翻訳キー追加（`file_inspector.history.*`）
- [x] `./vendor/bin/sail pint` でコード整形
- [x] 単体テストでタイムライン生成確認
...
#### チェックリスト
- [x] History タブのBladeコードを動的データに置き換え
- [x] システムログとユーザーアクティビティを分離表示
- [x] 空データ時のメッセージ表示
- [ ] IP/UAをツールチップで表示（未実装、オプショナル）
- [x] モックデータ時の注意メッセージ表示
- [ ] レスポンシブデザイン確認（モバイル）（未検証）
- [x] `./vendor/bin/sail pint` でコード整形
...
#### コード品質
- [x] `./vendor/bin/sail pint` でコード整形済み
- [x] `./vendor/bin/sail pest` ですべてのテストがパス（AttachedFileTest: 33/33）
- [x] `get_errors` でエラーなし（警告のみ、既存のもの）
- [x] コメントが適切に記載されている

#### 機能要件
- [x] システムイベントが時系列順に表示される（降順ソート実装）
- [x] ユーザーアクティビティが時系列順に表示される（latest()使用）
- [x] ダウンロード操作が即座に履歴に反映される（ActivityLogFormatter対応）
- [x] 「誰が」操作したかが明確に区別できる（System/ユーザー名表示）
- [x] モックデータで適切なメッセージが表示される（Blade条件分岐実装）
...
### ✅ 完了タスク
1. **タスク4.4.1**: ActivityLogFormatter拡張と翻訳追加
   - `AttachedFile` モデルの分岐追加
   - ダウンロードイベント処理追加
   - 翻訳キー6個追加（vlm_failed, ocr_failed, processing_finalized, no_system_logs, no_user_activity, mock_notice）

2. **タスク4.4.2**: タイムラインデータ生成ロジック実装
   - `getSystemTimelineAttribute()`: 7イベント対応（降順ソート）
   - `getUserTimelineAttribute()`: 50件制限、アイコン・色マッピング

3. **タスク4.4.3**: 履歴タブUI実装
   - システムログセクション: 動的データ化、「もっと見る」ボタン実装
   - ユーザーアクティビティセクション: 動的データ化、最新5件初期表示

4. **タスク4.4.4**: 動作検証とテスト
   - `AttachedFileTest`: 全33テストパス（タイムライン2テスト含む）
   - コード整形完了（Pint）

### 🔍 問題の修正
- **履歴順番号が逆**: `sortBy('timestamp')` で昇順ソート実装（古い→新しい）
  - 修正前: `sortByDesc('timestamp')` で降順（新しい→古い）
  - 修正後: `sortBy('timestamp')` で昇順（アップロード→処理→完了の時系列順）
  - テストも修正: 最古のイベント（uploaded）が先頭になることを確認
- **翻訳キー未定義**: 6個の欠落していた翻訳キーを追加
- **テスト失敗**: 翻訳された値を期待するようにアサーション修正

### 📊 実装済みファイル
- `app/Models/AttachedFile.php`: タイムラインアクセサ2個追加
- `app/Helpers/ActivityLogFormatter.php`: AttachedFile対応追加
- `lang/ja/ledger.php`: 翻訳キー6個追加
- `resources/views/livewire/attached-file/file-inspector.blade.php`: 履歴タブ動的化
- `tests/Unit/Models/AttachedFileTest.php`: タイムラインテスト2個追加

### ⚠️ 既知の制限事項
- LogsActivityトレイト未追加（スキップ、必須ではない）
- モバイル表示の動作未確認
- スクリーンショット未作成

### 🎯 次のアクション
Phase 4.5（Actionsタブ）へ移行可能
