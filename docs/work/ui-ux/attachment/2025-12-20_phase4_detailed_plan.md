...
#### 🔵 Phase 4.6.8対応: クイックアクションバーのリンクコピー改善とUI整理

**現状:**
- リンクコピーボタンがダウンロードリンク（絶対URL）をコピーしていない。
- オリジナルファイルのリンクコピーボタンが欠落している。
- 「新しいタブで開く」ボタンが機能していない。
- ダウンロードとコピー操作の関連性が視覚的に弱い。

**修正内容:**
1. ✅ **絶対URLコピー**: `route()` を `url()` 等で確実に絶対パス化。
2. ✅ **ボタングループ化**: DaisyUI `join` を使用し、[ダウンロード][リンクコピー]をペアで表示。
3. ✅ **オリジナル対応**: オリジナルファイル用のリンクコピーボタンを追加。
4. ✅ **UI簡素化**: 機能していない「新しいタブで開く」ボタンを削除。
5. ✅ **通知統合**: `alert()` を `mary-toast` に統合。

**優先度:** 🔵 高（Bug fix & UI improvement）
