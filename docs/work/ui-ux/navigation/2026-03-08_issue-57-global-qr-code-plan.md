...
- [x] **Task 1.1**: 翻訳ファイルの更新 (`lang/ja/ledger.php`)。
    - `page_qr_code` セクションの追加。
- [x] **Task 1.2**: Livewireコンポーネント `App\Livewire\Common\PageQrCode` の作成。
    - QRコード生成ロジックの実装。
- [x] **Task 1.3**: Bladeビュー `resources/views/livewire/common/page-qr-code.blade.php` の作成。
    - MaryUI モーダルとトリガーボタンの実装。
- [x] **Task 1.4**: `daisyuiNavigation.blade.php` への組み込み。

### Sprint 2: 管理画面適用とテスト (工数: 2h)
- [x] **Task 2.1**: Filament用ビューの調整（必要に応じて分岐または別ファイル化）。
- [x] **Task 2.2**: `AdminPanelProvider` への `renderHook` 登録。
- [x] **Task 2.3**: Feature Test の作成 (`tests/Feature/Livewire/Common/PageQrCodeTest.php`)。
    - コンポーネントのレンダリング確認。
    - QRコード生成の確認。

### Sprint 3: Filament管理画面の実装改善とUX向上 (工数: 2h)
- [x] **Task 3.1**: Filament用Livewireコンポーネントの分離または共通化の再検討。
    - `x-filament::modal` が正しく開かない、閉じない問題の修正。
    - アイコンボタンのトリガーを `x-on:click` から `$this->openModalAction` 形式（Filament Actions）へ変更検討。
- [x] **Task 3.2**: クリップボードコピーのUX改善。
    - コピー成功時にチェックアイコンを表示するフィードバックの実装。
    - ツールチップ（Filament標準）による通知の追加。
- [x] **Task 3.3**: Filament環境での動作確認エビデンスの取得。

### Sprint 4: バグ修正と再調整 (工数: 1h)
- [x] **Task 4.1**: FilamentでQRコードがローディング状態のままになる問題の修正。
    - サーバーサイドでのURL解決（Refererヘッダー活用）への移行により、JS連携の不安定さを解消。
- [x] **Task 4.2**: 各画面での閉じるボタン動作の修正。
    - MaryUI側は `@click="$wire.showModal = false"` によりプロパティ操作を確実に同期。
- [x] **Task 4.3**: クリップボードコピー時のUIフィードバックの正規化。
    - 成功時のチェックアイコン切り替えと最新URL取得ロジックの改善。
