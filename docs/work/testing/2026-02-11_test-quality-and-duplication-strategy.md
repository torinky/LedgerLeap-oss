...
現在、カバレッジ計測と Mutation Testing で対象範囲を使い分けています。

- **総合カバレッジ (`test:coverage`)**: 対象 = `app/` 全域
    - 「テストがどのコードを一度でも通ったか」を広く浅く確認するため、全域を対象にします。
- **Mutation Testing (`test:mutation`)**: 対象 = 指定した重要ロジック（主に `app/Services`, `app/Rules`, `app/Casts`）
    - 「テストがコードのロジック（意味）を正しく検証できているか」を深く確認します。

### 導入判断基準 (Inclusion Criteria)

以下の要件を満たすコードを Mutation Testing の対象とします。

1.  **ロジック密度が高い**: 条件分岐 (`if`, `switch`) や正規表現、アルゴリズムが複雑。
2.  **ドメインの重要性**: 権限管理、計算、データ整合性に直結する。
3.  **カバレッジが既に高い**: 既にテストが存在し、行カバレッジが概ね 80% 以上のものを「質」の段階へ引き上げる。

### ディレクトリ別判定

| ディレクトリ | 判定 | 理由 |
| :--- | :--- | :--- |
| **`app/Services`** | **必須** | 台帳計算、検索ロジック、ワークフロー等の核心ロジックが集約されているため。 |
| **`app/Rules`** | **必須** | バリデーションは微細な条件（> か >= か等）が重要なため、Mutation の恩恵が最大。 |
| **`app/Casts`** | **必須** | 「二重エンコード厳禁」ルールなど、データ破損に直結するシリアライズ制御を検証するため。 |
| **`app/Models`** | **部分的** | 基本は不要だが、`scopeSearch` などの複雑なクエリロジックを持つモデルは対象とする。 |
| **`app/Policies`** | **推奨** | セキュリティの要だが、単純な権限チェックのみのファイルは優先度を下げる。 |
| **`app/Livewire`**| **対象外** | 実行時間が極めて長く、ロジックは Services にあるべき。結合テストで担保。 |
| **`app/Http`** | **対象外** | Services の薄いラッパーであるべき。 |
...
## 7. Mutation Testing のトラブルシューティング
...
- **対策**: `infection.json5` の `timeout` を **900** （秒）程度に設定するか、実行時に `--timeout=900` を指定してください。
...
### D. データベースのユニーク制約エラーが発生する場合
**症状**: `UniqueConstraintViolationException` が発生する。
**原因**: テストデータベースのクリーンアップ（トランザクション）が不完全です。
**対策**: `RefreshDatabaseWithTenant` トレイトを使用している場合、`$connectionsToTransact` 配列に `mysql_testing` を含めるように修正してください。
...
### 各 Rule / Cast 用・最適化済みコマンド一覧
```bash
# UniqueAutoNumber (重要)
./vendor/bin/sail composer test:mutation -- --filter=app/Rules/UniqueAutoNumber.php --test-framework-options="--filter=UniqueAutoNumber" --map-source-class-to-test
# UniqueColumnValue
./vendor/bin/sail composer test:mutation -- --filter=app/Rules/UniqueColumnValue.php --test-framework-options="--filter=UniqueColumnValue" --map-source-class-to-test
# ValidAutoLinkPattern
./vendor/bin/sail composer test:mutation -- --filter=app/Rules/ValidAutoLinkPattern.php --test-framework-options="--filter=ValidAutoLinkPattern" --map-source-class-to-test

# AsColumnArrayJson (最重要: データ破損防止)
./vendor/bin/sail composer test:mutation -- --filter=app/Casts/AsColumnArrayJson.php --test-framework-options="--filter=AsColumnArrayJson" --map-source-class-to-test
# AsColumnDefinesArrayJson
./vendor/bin/sail composer test:mutation -- --filter=app/Casts/AsColumnDefinesArrayJson.php --test-framework-options="--filter=AsColumnDefinesArrayJson" --map-source-class-to-test
```
---
## 付録: 参考リンク
