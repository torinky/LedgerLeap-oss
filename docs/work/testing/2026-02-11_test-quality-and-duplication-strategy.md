...
## 5. 網羅対象の考え方（なぜ Services 限定なのか）

現在、カバレッジ計測と Mutation Testing で対象範囲を使い分けています。

- **総合カバレッジ (`test:coverage`)**: 対象 = `app/` 全域
    - 「テストがどのコードを一度でも通ったか」を広く浅く確認するため、全域を対象にします。
- **Mutation Testing (`test:mutation`)**: 対象 = `app/Services`
    - 「テストがコードのロジック（意味）を正しく検証できているか」を深く確認します。以下の理由から Services に限定しています。

### なぜ Services を優先するのか
1. **ビジネスロジックの集中**: LedgerLeap では、台帳の計算、検索ロジック、ワークフロー制御などの最も重要な判断が `Services` に集約されています。ここでの「1文字の誤り」は致命的なバグに直結します。
2. **実行コストの最適化**: Mutation Testing は数千回のテスト実行を伴うため、非常に時間がかかります。全域（Controllers, Livewire等）を対象にすると完了まで数時間を要し、開発サイクルが停滞します。
3. **テストの「質」の向上**: Controllers や UI (Livewire) のテストは、主に「期待通りの画面が出るか」などの結合確認が主目的です。一方、Services のテストは「純粋なロジック」の検証であり、Mutation Testing による「テストの穴」の発見が最も効果的です。

> [!TIP]
> 今後、独自バリデーションルール (`app/Rules`) や 独自の権限ロジック (`app/Policies`) など、ロジック密度が高いモジュールが追加された場合は、順次 Mutation Testing の対象に加えることを検討します。
