...
### ✅ ステップ 11: ワークフロー進捗の高度な可視化と複数必須ロール達成状況の表示 (完了)

* **目的:**
  フォルダに設定された複数の「必須点検ロール」および「必須承認ロール」について、現在のワークフローでどのロールが処理を完了したかを判定・表示する。進捗状況を直感的に把握できるUI（アイコン、バッジ、インジケーター、ツールチップ）を提供し、ユーザーが次のアクションを理解しやすくする。担当者選択UIもこの複数必須ロール情報を活用するように改修する。
* **方針:**
    * `Folder` に複数の必須点検ロールIDと必須承認ロールIDを中間テーブル (`folder_required_roles`) で関連付ける。
    * 「最後に内容が変更された `LedgerDiff` 以降のDiff履歴」を基準に、各必須ロールの処理状況を判定する。
    * 進捗状況は、タスクリスト、詳細画面に、状況に応じてアイコン、バッジ、メーター、ツールチップを組み合わせて簡潔に表示する。
    * 担当者選択UIでは、設定された複数の必須ロールのメンバーを優先的に提示し、各候補者の所属組織・保有ロールも表示する。
    *
    必須ロールの処理完了は、この段階ではシステム的な強制条件ではなく、あくまで「可視化」と「目安の提示」に留める。ただし、承認済み (
    `APPROVED`) ステータスの場合でも、必須ロールが未完了であればその旨をUI上で明示する。
* **実施済みタスク:**
    1. **DBスキーマ変更とモデル修正 (複数必須ロール設定):**
        * 中間テーブル `folder_required_roles` (`folder_id`, `role_id`, `type`) を作成。[完了]
        * `Folder` モデルに `requiredInspectorRoles()` と `requiredApproverRoles()` リレーション (Many to Many)
          を定義。[完了]
    2. **設定UI修正 (Filament `FolderResource` / Livewire フォルダ編集フォーム):**
        * フォルダ作成/編集フォームで、複数の必須点検ロールと複数の必須承認ロールを選択・保存できるようにUIを実装（
          `Select` multiple 等を使用）。中間テーブルへのレコード作成/削除を行う。[完了]
    3. **必須ロール処理状況判定ロジックの実装 (`Ledger` モデル):**
        * `Ledger` モデルに `getRequiredRolesProgressDetails(): array` メソッドを実装。[完了]
            * 比較基準Diff特定ヘルパー (`findBaselineDiffForProgressCheck`) を実装し、「最後に内容が変更された
              `LedgerDiff`」を特定。
            * フォルダ設定された必須点検/承認ロール群に対し、基準Diff以降の履歴を参照し、各ロールの処理状況（実行したユーザーがそのロールを持つか、適切なステータス変更か）を判定。
            * 結果を詳細な配列（総数、完了数、完了/未完了ロールリスト、全完了フラグなど）で返す。
    4. **UIへの進捗表示実装:**
        * **台帳詳細画面 (`Show.php` / `show.blade.php`):**
            * `getRequiredRolesProgressDetails()`
              の結果に基づき、点検と承認それぞれの進捗をメーター（プログレスバー）と数値で表示。ツールチップで完了済み/未完了のロール名を表示。[完了]
            * `APPROVED` ステータスで未完了の必須ロールがある場合に警告アイコン・メッセージを表示。[完了]
        * **タスクリスト (`PendingList.php` / `pending-list.blade.php`, `OtherRelatedTasksList.php` /
          `other-related-tasks-list.blade.php`):**
            *
            各タスク行のステータス表示の近くに、点検/承認の残り必須ロール数を小さなバッジやアイコン（完了/未完了を示す）で表示。ツールチップで詳細（完了/未完了ロール名）を表示。[完了]
    5. **担当者選択UIコンポーネント (`WorkflowAssigneeSelect`) の改修 (ステップ8で実施済み内容の流用・確認):**
        *
        フォルダの複数必須ロール情報を考慮し、該当ロールのユーザーを候補リストで優先的に表示するロジックを確認・調整。[完了]
        * 候補者の所属組織・保有ロール情報を表示するUIを確認・調整。[完了]
* **成果物:**
    * フォルダごとに複数の必須点検/承認ロールを設定できる機能。
    * 担当者選択時に、複数の必須ロール、実績、組織/ロール情報が考慮された、より適切な候補が提示されるUI。
    * 台帳詳細画面およびタスクリストで、必須ロールの処理進捗が直感的に可視化されるUI。
    * ユーザーがワークフローの残ステップやボトルネックを把握しやすくなった。
* **ドキュメント更新:** 上記の実施タスクと成果物を反映。[完了]

---

### ✅ ステップ 11.A: 担当者選択UIの高度化 (フォルダ必須ロール・実績・組織/ロール情報表示) (完了)

* **目的:** 点検・承認依頼時の担当者選択UIにおいて、フォルダに設定された「必須ロール」を考慮し、かつ過去の点検/承認実績があるユーザーを優先的に提示する。さらに、候補者の所属組織と保有ロールも表示し、選択の精度を向上させる。
* **方針:**
    * 担当者選択モーダル (`WorkflowAssigneeModal`) 内で呼び出される `WorkflowAssigneeSelect` コンポーネントを改修。
    * MaryUI の `Choices` コンポーネント（または Select のスロット）を活用し、リッチな情報表示と検索機能を提供する。
    * 候補者リストの生成ロジックを強化し、フォルダの必須ロール、過去の承認ルート（担当者シーケンス）、実績、権限、直近履歴を総合的に評価して表示順と情報を決定する。
    * 各推奨理由をアイコンで示し、モーダル内に凡例を表示する。
* **実施済みタスク:**
    1. **`WorkflowAssigneeSelect` コンポーネントのデータ取得ロジック強化:**
        * `fetchOptions(string $roleType, FolderPermissionType $requiredPermission)` メソッドを実装し、点検者/承認者候補リストを生成。
            * **A) フォルダ必須ロール担当者:** 対象フォルダの必須点検/承認ロールに所属するユーザーをリストアップし、理由「必須ロール」と高い優先度を付与。
            * **B) 実績ベース推奨ユーザー:** `WorkflowService::getFrequentAssignees`
              を利用し、実績多数ユーザーに理由「実績多数」と高い優先度を付与。必須ロールメンバーの実績も考慮。
            * **C) 直近の担当者履歴:** `getRecentAssignee` で直近担当者を取得し、理由「直近担当」と最優先度を付与。
            * **D) その他の権限保有ユーザー:** `UserService::getUsersWithFolderPermission`
              で権限を持つユーザーを取得し、理由「権限あり」と標準優先度を付与。
            * **E) 「過去の承認ルート」からの候補者:** `getPastRouteAssignees` で同一定義の過去完了案件の担当者を取得し、理由「過去ルート」と高い優先度を付与。
        * **候補者の追加情報取得と整形:** 各ユーザー候補について、主所属組織名、全保有ロール（ラベル）を `User`
          モデルのカスタム属性としてロードまたは動的生成。
        * **リスト統合とソート:** 上記情報を統合し、重複を除外。各ユーザーに複数の推奨理由と最終的な表示優先度を付与。
          `custom_reason_presentations` (アイコン名と凡例キーの配列) を各ユーザーオブジェクトに追加。優先度と名前でソート。
        * `searchAssignees(string $value = '')` メソッド: MaryUI `Choices` の `search-function` から呼び出され、検索文字列に応じて上記
          `fetchOptions` の結果をフィルタリングし、選択中のユーザーを必ず含めた上で `$options` (UserモデルのCollection)
          を更新。
    2. **`WorkflowAssigneeSelect` のビュー (`workflow-assignee-select.blade.php`) 修正:**
        * MaryUI の `<x-mary-choices>` コンポーネントを使用。
        * `@scope('item', $user)` スロット内で `<x-mary-list-item>`
          を使用し、各選択肢に「ユーザー名」「複数の推奨理由アイコン」「所属組織名」「保有ロールリスト」を整形して表示。
        * `@scope('selection', $user)` スロットで、選択されたユーザーの表示形式を定義。
    3. **親コンポーネントからの呼び出し修正 (`WorkflowAssigneeModal`):**
        * `WorkflowAssigneeModal` を開く際に、対象フォルダの必須ロール情報（ロールIDの配列）を `WorkflowAssigneeSelect`
          に渡すようにした。
    4. **凡例表示の実装:**
        * `WorkflowAssigneeSelect` に、各推奨理由コードに対応するHeroicon名と凡例用翻訳キーを返す静的メソッド
          `getAllReasonPresentations()` および `getStaticReasonPresentation()` を実装。
        * `WorkflowAssigneeModal` のビュー (`workflow-assignee-modal.blade.php`)
          内に、上記メソッドを呼び出して凡例（アイコンと説明テキスト）を表示するエリアを追加。
* **動作確認:**
    *
    担当者選択モーダルで、候補者リストがフォルダ必須ロール、実績、直近、過去ルート、権限などの情報を元に、適切な優先順位と理由アイコン付きで表示されることを確認。[完了]
    * 各候補者の所属組織名と保有ロールが簡潔に表示されることを確認。[完了]
    * 検索機能が動作し、リストが適切にフィルタリングされることを確認。[完了]
    * モーダル内に理由アイコンの凡例が表示されることを確認。[完了]
* **成果物:**
    * フォルダの必須ロール、過去の承認ルート、実績、ユーザーの所属組織・ロール情報が総合的に提示され、より文脈に沿った適切な担当者選択が可能になった、高度な担当者選択UI。
* **ドキュメント更新:** 上記の実施タスクと成果物を反映。[完了]

---

### ✅ ステップ 12: 必須ロール処理状況の LedgerDiff への累積記録と進行制御 (完了)

* **目的:** `LedgerDiff` に、現在の `Ledger`
  内容に対して処理が完了した必須点検ロールID群と必須承認ロールID群をJSONカラムとして累積的に記録する。これに基づき、ワークフローアクションボタンの表示/活性状態を制御し、各ステップ進行時に適切なチェックを行う。
* **方針:**
    * `LedgerDiff` テーブルに `completed_inspector_role_ids` (JSON) と `completed_approver_role_ids` (JSON) カラムを追加。
    * `Ledger` の `content` が変更された場合、これらのJSONカラムは新しい `LedgerDiff` で空配列として初期化される。
    * 内容変更がないステータス変更の場合、直前の `LedgerDiff` からこれらの配列を引き継ぎ、今回のアクションで完了したロールIDを追加する。
    * `Ledger` モデルの `getRequiredRolesProgressDetails()` は、`latestDiff`
      のこれらのJSONカラムと、フォルダに設定された全必須ロールを比較して達成状況を判定する。
    * アクションボタンの表示/活性は、この達成状況に基づいて行う。
    * `WorkflowService` の各アクションメソッドは、担当者の正当性に加え、「そのステップに進むために最低限必要な（いずれかの）必須ロール処理が完了しているか」を、この累積情報に基づいてチェックする。
* **実施済みタスク:**
    1. **DBマイグレーション (`LedgerDiff` テーブル修正):**
        * `ledger_diffs` テーブルに `completed_inspector_role_ids` と `completed_approver_role_ids` (JSON, nullable)
          カラムを追加。[完了]
    2. **`LedgerDiff` モデル修正:**
        * 上記カラムを `$fillable` と `$casts` (`array`) に追加。[完了]
    3. **`WorkflowService` メソッド修正:**
        * `saveDraft`, `saveEditedRecord` (内容変更時): 新しい `LedgerDiff` 作成時に `completed_*_role_ids`
          を空配列で初期化。[完了]
        * `requestInspection`: 直前のDiffから `completed_*_role_ids` を引き継ぐ（通常は空）。[完了]
        * `requestApproval` (点検完了→承認申請): 直前のDiffから `completed_approver_role_ids` を引き継ぎ、実行者の必須点検ロールIDを
          `completed_inspector_role_ids` に追加記録。[完了]
        * `approve` (承認): 直前のDiffから `completed_inspector_role_ids` を引き継ぎ、実行者の必須承認ロールIDを
          `completed_approver_role_ids` に追加記録。**この時点ではまだ `PENDING_APPROVAL` のままにし、最終承認条件はステップ13で判定。
          **[完了]
        * `returnToDraft`, `claimTask`: 直前のDiffの `completed_*_role_ids` を引き継ぐ。[完了]
    4. **`Ledger` モデルの `getRequiredRolesProgressDetails()` メソッド修正:**
        * `latestDiff` の `completed_inspector_role_ids` と `completed_approver_role_ids`
          を参照し、フォルダ設定された必須ロールと比較して、点検・承認それぞれの進捗（完了数、未完了ロールリスト、全完了フラグなど）を計算するようロジックを修正。[完了]
        * `findBaselineDiffForProgressCheck` メソッドは不要になったため削除。[完了]
        * ヘルパーメソッド `canProceedToApprovalStep()` と `canBeFinallyApproved()` を
          `getRequiredRolesProgressDetails()` の結果を参照するように修正。[完了]
    5. **アクションボタン表示/活性制御ロジックの実装 (UI側):**
        * 台帳編集/詳細画面 (`ModifyColumn.php`, `Show.php`)、タスクリスト (`PendingList.php`) で、
          `getRequiredRolesProgressDetails()` の結果や `canProceedToApprovalStep()` / `canBeFinallyApproved()`
          に基づき、「承認申請」「承認」ボタンの活性状態を制御。[完了]
            * 承認ボタンは、自分が担当者で、かついずれかの必須点検ロールによる点検が完了していれば活性化。
    6. **`WorkflowService` の進行条件チェックの調整:**
        * `requestApproval`: 実行者の正当性チェックを維持。
        * `approve`: 実行者の正当性チェックに加え、`latestDiff->completed_inspector_role_ids`
          が空でない（＝何らかの点検が完了している）ことをチェック。[完了]
* **成果物:**
    * `LedgerDiff` に、現在の内容に対する必須ロールの処理完了状況が累積的に記録されるようになった。
    * 最新の `LedgerDiff` を見るだけで、現在の内容に対する必須ロールの達成状況が判定可能になった。
    * UI上で、次に何をすべきかのヒント（どのボタンが押せるか）がより正確に提示されるようになった。
    * ワークフローの各ステップ進行が、より実態に即した条件で制御されるようになった。
* **ドキュメント更新:** 上記の実施taskと成果物を反映。[完了]

---

### ✅ ステップ 13: ワークフロー最終承認条件の厳格化 (完了)

*   **目的:** フォルダに設定された全ての必須点検ロールおよび必須承認ロールによる処理が完了していなければ、ワークフローが最終承認 (`APPROVED`) 状態に移行できないようにシステム制御を完全に実装する。
*   **方針:** `WorkflowService` の最終承認アクション (`approve` メソッド) 実行時に、`Ledger` モデルのヘルパーメソッド (`canBeFinallyApproved()`) を利用して必須ロールの処理状況を最終チェックし、条件未達の場合は専用の例外 (`WorkflowConditionException`) をスローして処理を中断、UIに具体的なエラーメッセージを表示する。
*   **実施済みタスク:**
    1.  **`WorkflowService::approve` メソッドの最終承認ロジック修正・強化:**
        *   `LedgerDiff` に実行者の必須承認ロール処理を記録した後、対象 `Ledger` の `canBeFinallyApproved()` メソッド（内部で `getRequiredRolesProgressDetails()` を利用し、全ての必須点検・承認ロールの完了を判定）を呼び出すようにした。[完了]
        *   もし `canBeFinallyApproved()` が `false` を返した場合（未完了の必須ロールが存在する場合）、`WorkflowConditionException` をスローし、「以下の必須ロールの処理が完了していません: [未完了の必須点検ロール名リスト], [未完了の必須承認ロール名リスト]」のような具体的なエラーメッセージを含めるようにした。[完了]
        *   全ての必須ロールが完了している場合のみ、`Ledger.status` を `APPROVED` に更新し、関連情報を記録（`LedgerDiff.status` を `APPROVED`、`LedgerDiff.approved_at` に日時、`LedgerDiff.approver_id` を最終承認者に設定）。[完了]
        *   最終承認に至らない中間承認の場合は、`Ledger.status` を `PENDING_APPROVAL` のままにし、次の承認者IDを `LedgerDiff.approver_id` に設定するロジックが維持されていることを確認。[完了]
    2.  **UIフィードバック (`Show.php` / `ModifyColumn.php`):**
        *   `approve` アクション実行時に `WorkflowService` から `WorkflowConditionException` がスローされた場合、その例外メッセージをキャッチし、ユーザーにトーストで分かりやすく表示するようにした。[完了]
        *   「承認」ボタンの表示/活性化ロジックはステップ12で実装済み（自分が担当者かつ最低限の点検が完了していれば活性）。最終的なブロックはService側の例外処理で行うため、UI上では押下可能だが押下後に条件未達エラーが出る形となることを確認。[完了]
    3.  **関連ドキュメント更新:** ワークフローの最終完了条件が厳格化され、必須ロールの全件処理がシステム的に担保されるようになったことを明記。[完了]
*   **動作確認:**
    *   フォルダに複数の必須点検・承認ロールを設定。
    *   全ての必須ロールの処理が完了していない状態で「承認」ボタンを押下（UI上は活性化されている場合）すると、`WorkflowService` で例外がスローされ、詳細なエラーメッセージ（どのロールが未完了か）がUIに表示され、ステータスが `APPROVED` にならないことを確認。[完了]
    *   全ての必須ロールの処理が完了した状態で「承認」ボタンを押下すると、ステータスが `APPROVED` になり、ワークフローが正常に完了することを確認。[完了]
    *   中間承認（他の必須承認ロールが残っている）の場合、ステータスが `PENDING_APPROVAL` のまま維持され、次の担当者が設定される（または選択を促される）ことを確認。[完了]
*   **成果物:**
    *   フォルダごとに定義された必須の承認プロセスがシステム的に完全に担保されるワークフロー。
    *   ユーザーが最終承認条件を満たしていない場合に、その理由が具体的なロール名と共に明確にフィードバックされるUI。
    *   途中の点検・承認の順序は柔軟性を保ちつつ、最終的な完了は全必須ロールの処理を経ることがシステム的に保証されるようになった。
*   **ドキュメント更新:** 上記の実施済みタスクと成果物を反映。[完了]

---

## 今後の展望 / 要検討事項

### LedgerLeap: ワークフロー機能 要検討事項 

以下は、現在までに実装されたワークフロー機能を踏まえ、今後の拡張や改善のために検討すべき事項です。

**I. ワークフロー定義とルートの柔軟性向上:**

1.  **複数点検者/複数承認者のより高度な管理 (現状の「複数必須ロール」の先):**
    *   **並列点検/承認:** 1つのステップで、複数の担当者（またはロール）のうち、何名が処理すれば次に進めるか（例: 3人中2人の承認でOKなど、いわゆる「M/N承認」）。
    *   **段階的点検/承認:** 同じ「点検」や「承認」という役割でも、複数の段階（例: チームリーダー点検 → 部長点検）を明確に定義・追跡できるようにするか。現状は「必須ロール」でカバーしているが、より明示的なステップ管理が必要か。
    *   **現状の課題:** 現在はフォルダに設定された「必須ロール」が全て処理されることを完了条件としているが、各ロール内では誰か一人が処理すればOKというOR条件に近い。これをAND条件（特定のロールグループ全員）やM/N条件にするか。
2.  **条件分岐ワークフロー (高度):**
    *   台帳の内容（例: 金額、カテゴリなど）に応じて、点検/承認ステップや担当者が動的に変わるような、より複雑なルート分岐を導入するか。
    *   **現状の課題:** 現在はフォルダ単位での固定的な必須ロール設定のみ。
3.  **点検ステップが不要なワークフロー:**
    *   台帳定義またはフォルダ設定で、「点検ステップをスキップし、申請後すぐに承認ステップに進む」オプションを提供するか。
    *   **現状の課題:** 点検ステップは常に存在する前提。
4.  **組織/プロジェクトをまたがる承認フローの正式サポート:**
    *   現状は、担当者選択UIでの手動選択による「擬似的な回付」に留まっている。
    *   A部門での承認後、B部門の特定ロールに自動的にタスクが移管され、B部門の承認プロセスが開始されるような、システム的な組織間連携ワークフローを実装するか。
    *   その場合の進捗管理や通知、権限の扱い。
5.  **承認ルートテンプレート:**
    *   頻繁に使用する点検・承認のパターン（担当者の組み合わせやステップ順）をテンプレートとして保存し、新規台帳定義作成時やフォルダ設定時に呼び出せるようにするか。

**II. 担当者と権限:**

1.  **代理承認/代理点検機能:**
    *   担当者が不在の場合に、権限を持つ別のユーザーが代理で処理を行える正式な仕組み。
    *   代理設定の方法（個人設定、管理者設定）、代理処理の記録、通知。
    *   **現状の課題:** 「タスク引き継ぎ」機能はあるが、これは恒久的な担当者変更に近い。一時的な代理処理とは異なる。
2.  **担当者の自動割り当て（オプション）:**
    *   特定の条件下（例: 申請者の所属組織、台帳のカテゴリなど）で、点検者や承認者をシステムが自動的に割り当てる（またはサジェストを強化する）機能。

**III. UI/UX と情報提示:**

1.  **差分表示の高度化:**
    *   現状はカラムごとの変更有無と前後の値表示。
    *   より詳細なテキスト差分（行単位、単語単位ハイライト）の導入検討。
    *   ファイルコンテンツの差分表示（可能な範囲で）。
2.  **アクションボタンへのインジケーター表示:**
    *   「点検依頼」「承認申請」ボタンなどに、「あと必須ロール〇件」のような具体的な情報をインジケーターとして表示し、ユーザーが次のアクションをより予測しやすくするか。
3.  **点検者による承認状況フォローの強化:**
    *   自分が点検を完了した案件について、その後の承認状況（誰が承認済み、誰待ちかなど）を専用のリストや通知で簡単に追跡できる機能。
    *   **現状の課題:** 「その他関連タスク」リストで自分が申請したものや引き継ぎ可能なものは追えるが、自分が点検した「だけ」のものは追いにくい。
4.  **通知の最適化とカスタマイズ:**
    *   各通知タイプ（特に `task_claimed` や、内容変更を伴う承認時など）のメール文面やシステム内通知の文面をさらにユーザーフレンドリーに。
    *   ユーザーが通知を受け取る条件やチャネルを、より細かくカスタマイズできるようにするか（フォルダごとの通知ON/OFFだけでなく）。
    *   通知カウンターの実装方法（DBポーリング、WebSocketなど）の最適化。
    *   ブラウザ通知の再検討（ネットワーク環境の制約が緩和された場合）。

**IV. 運用と管理:**

1.  **承認済み (`APPROVED`) レコードの変更・編集解除プロセス:**
    *   現状はロックされているが、特別な権限を持つユーザーが承認解除し、再度編集・ワークフローにかけるプロセスを定義・実装するか。その場合の監査証跡。
2.  **ワークフロー期限と督促機能:**
    *   各ステップの標準処理期限を設定し、期限が近づいている/超過しているタスクに対して担当者や管理者にリマインダー通知を送る機能。
    *   滞留案件の可視化。
3.  **Activity Log のさらなる活用:**
    *   現状は基本的なCRUD操作を記録。ワークフローの各アクション（担当者変更、コメント追加、ステータス変更など）をより詳細にActivity Logに記録し、監査証跡としての価値を高めるか。
    *   `LedgerDiff` が主要な履歴だが、Activity Logは補助的な全体操作履歴として利用。
4.  **ファイルアップロードとワークフロー:**
    *   台帳の添付ファイルだけでなく、ワークフローの各ステップで専用の添付ファイル（例: 点検時の指摘事項ファイル、承認時の参考資料）を追加・管理できるようにするか。
5.  **設定画面のUI/UX:**
    *   フォルダ設定画面（Filament / Livewire）での複数必須ロール設定UIの使い勝手向上。
    *   システム管理者とフォルダ管理者の設定権限の分離とUIの出し分け（もし必要なら）。
