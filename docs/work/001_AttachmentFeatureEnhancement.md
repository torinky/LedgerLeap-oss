...
* 	**構造:** `content_attached` カラムは、`content` カラムと同様に、**0から始まるインデックス配列**としてJSON形式で保存されます。この配列の要素の**順序**は、台帳定義の**カラムIDの昇順**と一致します。この対応付けは、保存直前に `LedgerDefine::normalizeByColumnDefine()` メソッドによって保証されます。このメソッドは、まず定義済みのカラムIDに基づいてデータの欠落を空の値で埋め、カラムIDでソートした後、最終的にキーを破棄して0から始まるインデックス配列に変換します。結果的にカラムIDと配列のキーが一致するスパースな配列になります。

```json
	// 例: 台帳定義に存在するカラムのIDが `1`, `2`, `4` のみの場合、
	// 正規化後の配列は5つの要素を持つ0ベースのインデックス配列となる。
	// インデックス `1` がカラムID `1` に、`2` が `2` に、`4` が `4` のデータに、それぞれ対応する。
	[
        "",
	    { "hashed_name_for_col_1.pdf": { "meta": { "content": "..." } } }, // index 1
	    { "hashed_name_for_col_2.png": { "meta": { "content": "..." } } }, // index 2
        "",
	    { "hashed_name_for_col_4.docx": { "meta": { "content": "..." } } } // index 4
	]
```
