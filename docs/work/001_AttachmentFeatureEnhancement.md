...
* 	**構造:** `content_attached` カラムは、`content` カラムと同様に、**0から始まるインデックス配列**としてJSON形式で保存されます。この配列の要素の**順序**は、台帳定義の**カラムIDの昇順**と一致します。この対応付けは、保存直前に `LedgerDefine::normalizeByColumnDefine()` メソッドによって保証されます。このメソッドは、まず定義済みのカラムIDに基づいてデータの欠落を空の値で埋め、カラムIDでソートした後、最終的にキーを破棄して0から始まるインデックス配列に変換します。
	```json
	// 例: 台帳定義に存在するカラムのIDが `1`, `5`, `10` のみの場合、
	// 正規化後の配列は3つの要素を持つ0ベースのインデックス配列となる。
	// インデックス `0` がカラムID `1` に、`1` が `5` に、`2` が `10` のデータに、それぞれ対応する。
	[
	    { "hashed_name_for_col_1.pdf": { "meta": { "content": "..." } } }, // index 0 (corresponds to column_id 1)
	    { "hashed_name_for_col_5.png": { "meta": { "content": "..." } } }, // index 1 (corresponds to column_id 5)
	    { "hashed_name_for_col_10.docx": { "meta": { "content": "..." } } } // index 2 (corresponds to column_id 10)
	]
	```
...
* 	`handle()` メソッド:
		...
		5.	**テキスト抽出失敗時:**
			...
			d.	**OCR対象外の場合 (ZIP, etc.):** これ以上処理できないため、`COMPLETED` に更新して処理を正常終了させる。
...
### 9. 添付ファイル表示と `content_attached` の問題と対応

#### 9.1. `ColumnHtmlService` での `attachmentContents` の表示問題

* 	**状態:** 完了
* 	**問題:** `app/Services/Ledger/ColumnHtmlService.php` の `getFileHtml()` メソッド内で、`$this->attachmentContents` が常に空となり、添付ファイルのツールチップに内容が表示されない問題が発生した。
* 	**原因:**
	* 	`resources/views/livewire/ledger/show.blade.php` の差分表示部分で、`ColumnHtml::setAttachmentContents()` に添付ファイルモデルのコレクション (`$change['current_attachments']` や `$change['old_attachments']`) を誤って渡していた。本来は `content_attached` から抽出したテキストコンテンツを渡すべきだった。
	* 	`resources/views/components/ledger/detail/table.blade.php` では、`$ledgerRecord->content_attached` を直接渡していたが、`ColumnHtmlService` の `getFileHtml()` メソッドが `content_attached` の内容をオブジェクトとしてアクセスしようとしていた（例: `$attachmentContents[$hashedFilename]->meta->content`）。しかし、`Ledger` モデルの `content_attached` は配列としてキャストされているため、オブジェクト形式でのアクセスが失敗していた。
* 	**対応:**
	* 	`app/Livewire/Ledger/Show.php` の `prepareContentDiff()` メソッドを修正し、`$contentChanges` 配列に、カラムIDごとに分割した `content_attached` の情報を `current_attachment_contents` および `old_attachment_contents` という新しいキーで追加した。
	* 	`resources/views/livewire/ledger/show.blade.php` を修正し、`ColumnHtml::setAttachmentContents()` に渡す値を、新しく追加した `$change['..._attachment_contents']` に変更した。
	* 	`app/Services/Ledger/ColumnHtmlService.php` の `getFileHtml()` メソッドを修正し、`$this->attachmentContents` の内容をオブジェクトとしてではなく、連想配列としてアクセスするように変更した（例: `$this->attachmentContents[$hashedFilename]['meta']['content']`）。
	* 	`resources/views/components/ledger/detail/table.blade.php` の `ColumnHtml::setAttachmentContents()` に渡す値を、`json_decode(json_encode(...))` によるオブジェクト変換を削除し、直接 `$ledgerRecord->content_attached` を渡すように戻した。

#### 9.2. `content_attached` カラムの上書き問題（再編集時）

* 	**状態:** 完了
* 	**問題:** 台帳の再編集時（特にファイル項目を触らずに他の項目のみを更新した場合）、`content_attached` に保存されていた添付ファイルの抽出済みテキストが空の配列で上書きされ、データが消失する不具合が発生した。
* 	**原因の特定と調査の経緯:**
	1.	**初期仮説:** `WorkflowService` でのDB更新処理に問題がある可能性を疑い、ログを追加して調査。しかし、Serviceに渡される時点で既に `content_attached` が空になっていることが判明。
	2.	**Livewireコンポーネントの調査:** 問題のスコープを `app/Livewire/Ledger/ModifyColumn.php` に絞り込み。`mount()` メソッドで `content_attached` がロードされていないことを発見し修正したが、問題は解決せず。
	3.	**ログの再分析:** `spatie/laravel-activitylog` のログを詳細に分析した結果、Eloquentの `updated` イベントが発火する時点で `content_attached` が空になっていることを確認。これにより、`WorkflowService` よりも前の、Livewireコンポーネントの処理中にデータが失われていることが確定した。
	4.	**根本原因の特定:** `ModifyColumn` の親クラスである `CreateColumn` を調査したところ、`content_attached` プロパティが `private` で宣言されていた。子クラスの `ModifyColumn` で `public` に変更しようとしても、PHPの仕様上、可視性をより厳しくすることしかできないため、実質的に `private` として扱われていた。Livewireは `public` プロパティの状態しかリクエスト間で維持しないため、ユーザーが画面上で何らかのアクション（テキスト入力など）を行うたびに `content_attached` プロパティが空の配列 `[]` で再初期化され、その結果、空のデータがDBに保存されていた。
* 	**対応:**
	1.	**`app/Livewire/Ledger/CreateColumn.php` の修正:** 親クラスの `$contentAttached` プロパティの可視性を `private` から `public` に変更した。
	2.	**`app/Livewire/Ledger/ModifyColumn.php` の修正:** 子クラスで重複して定義されていた `$contentAttached` プロパティの宣言を削除した。
* 	**結果:** 上記の修正により、Livewireコンポーネントのライフサイクルを通じて `content_attached` の状態が正しく維持されるようになり、データ消失の不具合は完全に解消された。
