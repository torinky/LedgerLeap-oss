...
台帳定義のリスト表示、編集時、詳細表示での説明欄にマークダウン形式で注意事項やリンク先を追加しました。
...
## 関連リンク
- [営業マニュアル](https://example.com/sales-manual)
...
DateTypeクラスに日付のデフォルト値設定機能を追加しました。

#### 実装内容

1. **オフセット設定機能**
   - `options['defaultOffset']`: 相対的な日数オフセット（例: "0d", "7d", "-1M"）
   - `options['overwriteOnEdit']`: 再編集時にデフォルト値で上書きするか（true/false）

2. **パース処理**
   - `DateType::parseDefaultOffset()`: オフセット文字列をパース
     - `d`: 日
     - `w`: 週
     - `M`: 月
     - `y`: 年
...
3. **デフォルト値計算**
   - `DateType::getDefaultValue()`: 現在日時からオフセット分を計算
   - 空欄の場合はnullを返す（デフォルト日設定なし）

4. **UI連携**
   - 台帳定義編集画面で「デフォルトオフセット」入力欄を追加
   - バリデーション: 正規表現 `/^[+-]?\d+[dwMy]$/`
   - 入力フォームで自動的にデフォルト日が設定される

#### テスト

- ✅ `DateTypeTest`: オフセットパース、デフォルト値計算のユニットテスト（15テスト全てパス）
- ✅ `ModifyColumnDateTypeTest`: UI・バリデーション・保存のFeatureテスト（10テスト全てパス）
- ⚠️ `DateDefaultInitializationTest`: 入力フォームでのデフォルト値設定テスト（テナント設定必要）

### カラム定義の表示設定

1. **表示レベル（displayLevel）**
   - 既存機能を活用
   - 詳細度に応じた表示制御

2. **表示グループ（displayGroup）**
   - 既存機能を活用
   - カラムのグループ化
