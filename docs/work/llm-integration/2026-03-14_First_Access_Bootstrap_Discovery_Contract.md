...
5. **UI evaluation plan for local LLM + Continue**
   ...
   - Tracking Issue: #96
6. **Client skill initialization gate**
   - bootstrap discovery の後に client-side skill の有効化完了を確認し、pre-init では bootstrap 系だけを許可する
   - Tracking Issue: #101
...
## 11. 補足提案（2026-03-14 / initialization gate）

Sprint 6 では discovery contract を固定したが、
Issue #83 の後続検討として、**discovery の後に initialization gate を置く案** が追加で有力になった。

この案では次を区別する。

- discovery: 何を入れるべきかを返す
- initialization: client 側で必要 skill を有効化する
- gate: 初期化完了まで通常 tool を閉じる

重要なのは、これは `optional export` と同義ではないことである。
実ファイル生成は optional のままでも、**初期化完了確認そのもの** は required にできる。

この補足提案は `2026-03-14_Client_Skill_Initialization_Gating_Proposal.md` で別途整理する。

## 10. 実装反映メモ（2026-03-14）

- Issue #93 により `bootstrap-client-skills` prompt を実装
- MCP から `client_type` / `role_profile` / `model_profile` / `language` を受けて、最初の質問例 / 確認事項 / 次アクションだけを返す開始支援 prompt を取得可能になった
- Prompt は補助導線に限定し、dynamic bundle 解決・file 配布・developer-facing internals は扱わない
...
- Issue #95 では optional file export / package generation を **既存 CLI ベースの downstream export** として整理した
- `ai:bootstrap-client-skills` は discovery の代替契約ではなく、`files` / `placement_instructions` を実ファイル化する補助導線として扱う
- Issue #95 時点の package は zip ではなく **client 別ディレクトリ生成** を指し、archive 配布や MCP export tool は follow-up 候補に分離した
