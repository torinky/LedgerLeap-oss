...
|---|---|---|---|---|---|
| Resource | `ledgerleap://bootstrap/{client}` | 初回導線の短い参照カード | client 別の概要、導入順、ガイド参照先 | role / model / client 依存の動的 bundle 解決 | **候補維持**。静的 bootstrap card として有効 |
...
- static resource (`ledgerleap://bootstrap/{client}`) と bootstrap prompt starter (`bootstrap-client-skills`) は引き続き別 Issue（#92 / #93）の責務とする
