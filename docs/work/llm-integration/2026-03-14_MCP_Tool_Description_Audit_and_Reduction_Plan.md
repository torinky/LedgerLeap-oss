...
## 14. Issue #100 実装トラッカー（2026-03-14）

### 14.1 受け皿を先に厚くしたファイル

| ファイル | 追加・維持した client-facing 情報 | 理由 |
|---|---|---|
| `resources/ai/capabilities/ledger-search.yaml` | keyword → semantic → activity log pivot、meta の読み方、related-record flow、`include_content=false` / `mode=count` による段階的取得、「重要」の見方、追加 examples | `SearchLedgersTool` から削る guidance の一次受け皿を先に確保するため |
| `resources/ai/capabilities/ledger-update.yaml` | 一覧だけで即更新しない前提、single-record read、schema 確認、`dry_run`、`DRAFT` 戻り、`APPROVED` lock、tag 未対応 | `GetLedgerDetailTool` / `UpdateLedgerTool` の標準フローを capability 側へ集約するため |

### 14.2 slim 化した tool / server と移送先

| ファイル | description / instructions から除去したもの | 残した契約説明 | 移送先 |
|---|---|---|---|
| `app/Mcp/Tools/GetLedgerDetailTool.php` | Search → Detail → Defines → dry_run → Update の `Standard workflow` | 単一レコードの最新内容、workflow state、editability、`summary/raw` | `resources/ai/capabilities/ledger-update.yaml` |
| `app/Mcp/Tools/UpdateLedgerTool.php` | update の順序付き `Standard workflow` | `content_patch`、`comment`、`dry_run`、tag 未対応、`APPROVED` lock | `resources/ai/capabilities/ledger-update.yaml` |
| `app/Mcp/Tools/GetRelatedLedgersTool.php` | 検索 → 詳細 → 関連探索 → 詳細確認の手順 | source ledger 起点、identifier / semantic の二軸、`limit`、`summary/raw` | `resources/ai/capabilities/ledger-search.yaml` |
| `app/Mcp/Tools/GetClientBootstrapManifestTool.php` | REST parity、shared service reuse、internals 非露出の実装背景 | `client_type` / `role_profile` / `model_profile` / `language`、返却 bundle 項目 | `docs/work/llm-integration/2026-03-14_First_Access_Bootstrap_Discovery_Contract.md` |
| `app/Mcp/Tools/SearchLedgersTool.php` | Strategic Usage、fallback、Common Search Patterns、Important / Sorting / Performance / Workflow Example | 検索対象、主要パラメータ、`summary/raw`、`meta`、`semantic_score`、日本語対応の注意 | `resources/ai/capabilities/ledger-search.yaml` |
| `app/Mcp/Servers/LedgerLeapServer.php` | 日付検索や analytics の step-by-step guidance、期間候補の羅列 | `__summary__` / `__display_fields__` の扱い、日本語応答、list→detail の表示原則 | capability manifest / bootstrap discovery docs |

### 14.3 実装後に維持したガードレール

1. **削除だけの変更を作らない**
   - tool description から削る段落は、必ず capability manifest または developer-facing docs に移送先を持たせる。
2. **tool は契約、flow は capability**
   - `description` には他 tool を列挙した標準手順を書かず、標準業務フローは `resources/ai/capabilities/*.yaml` に寄せる。
3. **developer-facing 背景は docs/work に隔離**
   - REST parity や shared service reuse の背景は tool へ戻さず、判断ログと discovery docs に残す。
4. **Search は最後に slim 化**
   - 一番情報量の多い `SearchLedgersTool` は、manifest 側の受け皿を補強した後に短文化した。
