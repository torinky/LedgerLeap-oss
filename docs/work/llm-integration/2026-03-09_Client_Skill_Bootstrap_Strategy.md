...
**ステータス:** Sprint 1-3 完了 / Sprint 4 以降は未着手
...
> **現状メモ:** Sprint 1（情報設計のリセット）と Sprint 2（client-facing capability taxonomy）は 2026-03-10 に完了。Sprint 3（developer-facing maintenance taxonomy）は 2026-03-12 に完了し、内部制約の保守先・`.github` / `docs/work` / generator prototype の境界・SoT / 派生物の関係を整理しました。Sprint 4 以降は Issue #83 配下の候補で、**まだ着手しません。**
...
- **初回アクセス時に client 環境へ skill を生成・配置しやすい discovery 契約が定義される**
...
### 完了条件
- MCP 側の候補（resource / prompt / tool）が整理されている
...
- **Sprint 1-3 は完了管理済みで、Sprint 4 以降の個別 Issue はまだ着手しない。**
- Sprint 4〜6 は Issue #83 配下の候補であり、着手判断は別途行う。
...
- [ ] **初回アクセス時の bootstrap discovery 契約が定義されている**
