...
### 注意点
- `AuthenticatedMcpTool` の env token 前提を見直す必要がある
...
- `routes/ai.php` で `Mcp::web(...)` をどう公開するか決める
- `auth:sanctum` と header auth の整合を定義する
- `AuthenticatedMcpTool` の env token 依存を整理する
