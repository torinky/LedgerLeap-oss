...
*   **目的:**
    まず、閲覧のみである詳細表示画面に、カラム定義の「グループ名」に基づいたグループ化・折りたたみ表示を実装します。これにより、グループ化機能の基本コンポーネントとロジックを確立します。
*   **対象ファイル:**
    *   `app/Livewire/Ledger/Show.php`
    *   `resources/views/livewire/ledger/show.blade.php`
    *   `resources/views/components/ledger/detail/table.blade.php`
*   **実装タスク:**
    1.  **`Show.php` の改修:**
        *   グループの開閉状態を管理する `public array $collapsedGroups = [];` プロパティを追加します。
        *   グループの開閉をトグルする `toggleGroup(string $groupName)` メソッドを実装します。
        *   `render()` メソッド内で、表示対象のカラムを「グループ名」でグループ化し、ビューに渡します。
        *   `mount()` で、必須項目を持つグループがデフォルトで開くように `$collapsedGroups` を初期化します。
*   **確認方法:**
    *   台帳詳細画面で、項目がグループごとに折りたたまれて表示されること。
    *   グループ名をクリックすると、内容が開閉すること。
    *   差分表示に切り替えても、グループ化が維持され、差分が正しくハイライトされること。
