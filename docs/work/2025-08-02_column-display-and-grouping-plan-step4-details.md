...
「重要度（表示レベル）」と「グループ名」という概念を、台帳の詳細画面および入力フォームに適用します。これにより、長大なフォームや情報過多な画面を整理し、ユーザーがより直感的かつ効率的に情報を操作できるようにします。

## 3. 実装計画

### **ステップ4.1: 詳細画面 (`Show`) の改修**

*   **目的:** 台帳の詳細画面において、カラムを「グループ名」でグループ化し、折りたたみ可能なパネルで表示する。また、表示レベルに応じて表示/非表示になるカラムを制御する。
...
*   **主なタスク:**
    1.  Livewireコンポーネントに、現在の表示レベルを管理するプロパティを追加する。
...
*   **詳細設計:**
    *   **`app/Livewire/Ledger/Show.php` の改修:**
        *   **プロパティの追加:**
        ...
        *   **カラムのグループ化とフィルタリングロジックの追加:**
            *   `render()` メソッド内で、`$this->ledgerRecord->define->column_define` をグループ名でグループ化し、各グループ内のカラムを `display_level` に基づいてフィルタリングするロジックを追加します。
            *   グループ化されたカラムは、ビューに渡すための新しいプロパティ（例: `$groupedColumns`）に格納します。
            *   グループ名が `null` のカラムは「その他」グループとして扱います。

    *   **`resources/views/livewire/ledger/show.blade.php` の改修:**
        *   **表示レベル切り替えボタンの設置:**
...
*   **理由と意図:**
    *   **視認性の向上:** 長大なフォームをグループ化し、折りたたみ可能にすることで、ユーザーは必要な情報に素早くアクセスでき、フォーム全体の把握が容易になります。
    *   **操作性の向上:** 表示レベルを切り替えることで、ユーザーは自分の関心のある粒度で情報を閲覧でき、不要な情報を非表示にすることで、画面のノイズを減らし、集中力を高めます。
    *   **パフォーマンスの最適化:** 非表示のカラムはDOMから削除されるため、ページのレンダリング負荷が軽減されます。
    *   **UIの一貫性:** `RecordsTable` と同様の表示レベル切り替え機能を提供することで、システム全体のUIに一貫性を持たせます。
    *   **段階的な実装:** 詳細画面は主に閲覧が目的であるため、入力フォームよりも先に実装することで、機能の検証を段階的に進めることができます。
...
*   **確認方法:**
    1.  台帳の詳細画面にアクセスし、カラムがグループ化され、折りたたみ可能なパネルで表示されることを確認する。
