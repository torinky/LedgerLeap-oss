...
**検証コマンド**
```bash
./vendor/bin/sail test --testsuite=Unit --exclude-group=external,database-migrations
./vendor/bin/sail test --testsuite=Feature --exclude-group=external,database-migrations
./vendor/bin/sail test --group=database-migrations
```
...
| - [ ] 直列ジョブ用DBとの責務境界を定義 | In Progress |
| - [ ] Actions ベースライン（job別時間）を採取 | Not Started |
| - [ ] `concurrency` / `paths-ignore` 適用可否を判定 | Not Started |

### Sprint 3 管理タスク
| Task | Status |
|---|---|
| - [ ] Actions 重複セットアップ削減方針を確定 | Not Started |
| - [ ] `unit` / `feature` / `db-migrations` 互換を維持して短縮化 | Not Started |
| - [ ] `RefreshDatabaseWithTenant` 改修前提を確定 | Not Started |
| - [ ] ロールバックRunbookを整備 | Not Started |
| - [ ] 変更結果を Issue #81 に反映 | Not Started |
...
| - [ ] フレーク率/実行時間の記録フォーマットで判定 | Not Started |
...
- `unit` / `feature` ジョブは `--exclude-group=external,database-migrations` を維持する
