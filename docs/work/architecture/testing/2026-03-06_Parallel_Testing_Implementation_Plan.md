...
| Task | Status |
|---|---|
| - [ ] CI標準を `parallel + serial` 2系統に確定 | Done |
...
| - [ ] CI ワークフローにワーカーDB事前作成ステップを追加 | Done |
| - [ ] `switchToTestingDatabase()` / `getBaseDatabaseName()` 等の不要メソッドを削除 | Done |
...
### Sprint 6 追加整理ログ（2026-03-08）
- `tests/Traits/RefreshDatabaseWithTenant.php` から `getBaseDatabaseName()` / `setBaseDatabaseName()` / `clearBaseDatabaseName()` を削除
- `switchToTestingDatabase()` を廃止し、`mysql_testing` の接続切替を呼び出し箇所へ直接化
- 検証:
  - `./vendor/bin/sail test tests/Feature/Livewire/Ledger/LedgerDiffViewerTest.php tests/Feature/Livewire/Ledger/CreateColumnInputTypeValidationTest.php` → `13 passed (61 assertions)`
  - `./vendor/bin/sail pest --parallel --processes=2 --filter="LedgerDiffViewerTest|CreateColumnInputTypeValidationTest" --testsuite=FeatureParallelSubset --display-errors` → `13 passed (61 assertions)`
- Canary 現況: 直近 2 run（`22798967130`, `22798966638`）は Unit / Feature subset ともに GREEN。10 連続成功は未達のため Sprint 7 で継続確認

### Sprint 7 標準化ログ（2026-04-05）
- Composer scripts を `test:ci` / `test:ci:serial` / `test:ci:parallel` に整理し、`test:full` は `prepare-local -> ci -> external` に更新
- PhpStorm の Run Configuration に `CI: Standard` と `Pest: Parallel (phpunit.parallel.xml)` を追加し、serial / parallel の入口を分離
- 既存の `CI: Unit` / `CI: Feature` / `CI: DB Migrations` と `Parallel: Unit` / `Parallel: Feature Subset` は、標準 split の個別起動入口として維持
