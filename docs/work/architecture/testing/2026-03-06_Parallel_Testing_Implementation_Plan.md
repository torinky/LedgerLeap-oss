...
### Sprint 5 実施ログ（2026-03-06）
- `docs/work/architecture/testing/2026-03-06_Parallel_Testing_Developer_Guide.md` を新規作成
  - ローカル日常コマンド / グループ分類ルール / CI ジョブ構成 / トレイト使用ガイド / ロールバック簡易版 を統合
- Issue #81 本文を更新（Sprint 3/4 チェックボックスを完了に変更、Sprint 5/6 タスクを追記）
- CI標準: `phpunit.yml`（直列）+ `parallel-canary.yml`（カナリア並列）の2系統で運用中
- KPI確定はカナリア 10 連続成功後に実施予定
...
1. `refreshDatabase()` に並列実行時の分岐を追加
   - `ParallelTesting::token()` が非null（ローカル並列）: `migrate:fresh` を実行せず、ワーカーDB（`ledgerleap_test_test_{token}`）に切り替えて `migrate --force` のみ実行
   - CI（`env('CI')`）: 従来通りスキップ
   - ローカル直列（token=null）: 従来通り `migrate:fresh`

2. `switchToTestingDatabase()` を新規追加
   - `$_SERVER['DB_DATABASE']` / `$_ENV['DB_DATABASE']` を書き換えて artisan 再ロード時のconfig上書きを防止
   - `DB::purge('mysql_testing')` + `config()->set()` でワーカーDB名に切り替え

3. `LedgerDiffViewerTest`: `RefreshDatabase` → `RefreshDatabaseWithTenant` に移行、`#[CoversClass]` 追加

**検証結果**
```
# ローカル並列実行
./vendor/bin/sail pest --parallel --recreate-databases --testsuite=FeatureParallelSubset \
  --exclude-group=external --exclude-group=database-migrations
→ Tests: 2 skipped, 479 passed (1174 assertions)  Duration: 191.65s  Parallel: 8 processes

# ローカル直列実行
→ Tests: 2 skipped, 479 passed (1174 assertions)  Duration: 707.52s
```
