...
3.  **ナビゲーションUIの問題:**
    ...
    *   **関連資料:** [テナント・フォルダスイッチャーUI改善計画書](./2025-09-14_tenant-folder-switcher-ui-improvement-plan.md) <span style="color: green;">完了</span>
    *   **状況:** 「テナント・フォルダスイッチャーUI改善計画書」の記載に基づき、本課題は完了済み。MaryUIでの試行錯誤を経て、DaisyUIの `<details>` タグと `<ul>`/`<li>` を活用したアコーディオン形式のメニューが最終的に採用され、幅の自動調整、縦方向の拡張、アクティブ表示、LocalStorageによる開閉状態の維持などの改善が施された。また、Filament管理パネルへのテナント切り替えメニューの追加も完了している。
4.  **ユーザー管理の仕様・運用上の課題:**
...
6.  **AutoLink機能のマルチテナント対応不足（システムグローバルな改修）:**
    ...
    *   **関連資料:** [自動リンク機能 マルチテナント対応計画 (ハイブリッドアプローチ)](./2025-09-15_auto-link-multi-tenant-hybrid-plan.md)
