...
### Phase 2: UI統合（0.5週間） 🟡 MVP推奨

**目標:** デフォルトソートに複合スコアを適用し、既存機能との共存を確認

#### Step 2.1: Livewireコンポーネント更新（1日）

- [ ] `RecordsTable.php`のデフォルトソートを`composite_score DESC`に設定
- [ ] カラムヘッダークリックで従来のソートに切り替え可能にする
- [ ] 現在のソート状態を視覚的に表示

#### Step 2.2: パフォーマンス測定（0.5日）

- [ ] 大量データ（1万レコード）でのレスポンスタイム測定
- [ ] スコア計算のN+1問題をチェック
- [ ] 必要に応じてEager Loadingを追加

#### Step 2.3: 統合テスト（0.5日）

- [ ] 検索 + フィルタ + スコアソートの組み合わせテスト
- [ ] マルチテナント環境でのテスト
- [ ] ブラウザテスト（Dusk）
