...
#### 問題点B: Tikaによるテキスト抽出機能の欠落 (解決済み・仕様)

-   **当初の懸念:** `app/Jobs/Ledger/ExtractTextWithTika.php` ファイルが存在せず、Office文書などのテキスト抽出機能が欠落していると懸念された。
-   **調査結果:** `ProcessAttachedFile` ジョブの現在の実装を調査した結果、このジョブがTikaクライアントを**直接呼び出し**、あらゆるファイル形式のテキスト抽出を試みる設計になっていることが確認された。
-   **結論:** `ExtractTextWithTika` という個別のジョブファイルが存在しないのは、バグではなく**現在の仕様**である。`ProcessAttachedFile` がテキスト抽出の責務をすべて担っているため、コードの修正は不要と判断した。

#### 問題点C: Observer実装の欠落とジョブの二重ディスパッチ (解決済み)

-   **当初の懸念:** `AttachedFile` モデルの `created` イベントと `ProcessAttachedFile` ジョブの両方から `GenerateThumbnail` ジョブがディスパッチされ、処理が重複する可能性が指摘された。
-   **調査結果:** `ProcessAttachedFile` ジョブは、Tikaによって正確なMIMEタイプが判明した後にディスパッチするため、こちらを正とした。モデル作成直後の `created` イベントでのディスパッチは、タイミングとして不適切であり冗長であると判断した。
-   **結論:** `AttachedFile` モデルの `booted()` メソッド内にあった `created` イベントリスナーを削除し、`ProcessAttachedFile` ジョブからのディスパッチに一元化した。これにより、**ジョブの二重ディスパッチ問題は解決された。**
