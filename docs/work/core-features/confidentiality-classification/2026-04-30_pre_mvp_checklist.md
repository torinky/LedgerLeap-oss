...
## 10. Sprint 2 完了後の未確定事項（Sprint 3 で決定が必要）

### 10.1 フォームのバリデーション
- FolderForm には `confidentialityLevel` の `in:public,internal,confidential,secret` ルールが設定済み
- LedgerDefineEdit にはバリデーションが未設定（Sprint 3 で統合時に必要）
- `confidentialityScopes` のバリデーション（`exists` チェックの対象テーブル・カラム）

### 10.2 選択コンポーネント
- `x-mary-choices` vs `x-mary-choices-offline` の最終決定
- 組織・ロールの混在リストに対するグルーピング表示の有無
- 略称 vs 正式名の表示優先順位

### 10.3 保存時の JSON 正規化
- `ConfidentialityLevelService::allScopes()` は `id` が `"org:1"` / `"role:3"` の形式
- 保存時に `"org:"` / `"role:"` プレフィックスを除去して `{"org_ids":[{"id":1,"name":"..."}],...}` に変換する必要がある
- 変換ロジックを Service に追加するか、Livewire コンポーネントで行うか

### 10.4 スタンプの動的表示
- IndexManager / edit / show にスタンプを動的データで有効化
- Intersection Observer の実装方法（Livewire + Alpine.js の連携）
- スタンプ非表示時（`level` が `null`）のレイアウト崩れ防止
