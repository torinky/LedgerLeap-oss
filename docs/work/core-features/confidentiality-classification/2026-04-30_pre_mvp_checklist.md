...
**決定済み（2026-05-03）:**
- [x] **使用コンポーネント**: `x-mary-choices-offline` で確定（Sprint 3 で実装済み）
  - `FolderForm`・`LedgerDefine/Edit`・`LedgerDefine/Create` すべてで `x-mary-choices-offline` を使用
  - 理由: 組織・ロールは全件メモリに乗る規模であり、オフライン選択で十分な応答性
- [x] **選択肢のグルーピング**: フラットリストで確定
  - `ConfidentialityLevelService::allScopes()` は `$organizations->merge($roles)->values()->all()` でフラットに返す
  - 各選択肢に `type` フィールド（`organization` / `role`）を持たせ、UI 側で区別可能
- [x] **表示項目**: 略称優先で確定
  - 組織: `$org->abbreviation ?? $org->name`
  - ロール: `$role->abbreviation ?? $role->description ?? $role->name`
  - 略称が未設定の場合は正式名にフォールバック
...
**決定済み（2026-05-03）:**
- [x] **カスタムログメッセージ**: MVP では `LogsActivity` のデフォルト動作（`logFillable()`）で運用。`confidentiality_level`・`confidentiality_scopes` は `$fillable` に含まれているため、変更差分は自動記録される
- [x] **Phase 2 で検討**: JSON 差分を人間が読める形式にカスタムメッセージ化する必要が運用フィードバックで判明した場合、Phase 2 で `getDescriptionForEvent()` の拡張または `ActivityLog` の tap 処理を検討
...
**決定済み（2026-05-03）:**
- [x] **公開範囲DBベース化に伴うテスト追加**: 以下のテストファイルで実装済み
  - `tests/Feature/Livewire/Folder/FolderFormTest.php`（秘密区分保存・更新・初期化）
  - `tests/Feature/Livewire/LedgerDefine/EditTest.php`（秘密区分保存・クリア）
  - `tests/Feature/Livewire/LedgerDefine/CreateTest.php`（作成時秘密区分設定）
  - `tests/Feature/Components/ConfidentialityStampTest.php`（レンダリング・リンク）
  - `tests/Unit/Services/ConfidentialityLevelServiceTest.php`（継承・パス・フォールバック）
  - `tests/Feature/Filament/OrganizationResourceTest.php` / `RoleResourceTest.php`（略称 UI）
- [x] **テスト用キャッシュ**: 各テストメソッドの先頭で `Cache::flush()` を実行し、キャッシュ汚染を防止する方針で確定（`setUp()` への統一は将来のリファクタリング候補）
...
## 10. Sprint 2 完了後の未確定事項 → 全て Sprint 3 / 3A で決定済み

### 10.1 フォームのバリデーション ✅（Sprint 3 で完了）
- [x] FolderForm: `confidentialityLevel` に `in:public,internal,confidential,secret` ルールを設定済み
- [x] LedgerDefineEdit: `confidentialityLevel`・`confidentialityScopes` のバリデーションを設定済み
- [x] LedgerDefineCreate: 同様のバリデーションを設定済み
- [x] `confidentialityScopes` の変換: `ConfidentialityLevelService::parseScopeChoices()` で `"org:1"` → `{"org_ids":[{"id":1}]}` に正規化

### 10.2 選択コンポーネント ✅（Sprint 3 で完了）
- [x] `x-mary-choices-offline` で確定（§2.2 参照）
- [x] フラットリスト + `type` フィールドで確定
- [x] 略称優先表示で確定

### 10.3 保存時の JSON 正規化 ✅（Sprint 3 で完了）
- [x] `ConfidentialityLevelService::parseScopeChoices()` を Service 側で実装済み
- [x] Livewire コンポーネント（FolderForm / LedgerDefineEdit / LedgerDefineCreate）から `parseScopeChoices()` を呼び出して保存

### 10.4 スタンプの動的表示 ✅（Sprint 3 / 3A で完了）
- [x] IndexManager / edit / show にスタンプを動的データで有効化済み
- [x] Intersection Observer API によるスクロール連動スタンプ表示を実装済み（`records-table.blade.php`）
- [x] スタンプ非表示時（`level` が `null` または `public`）は何も描画しないためレイアウト崩れなし
...
### ブランチ
- **Sprint 2 完了ブランチ**: `feature/confidentiality-classification-sprint2`
- **Sprint 3 開始時**: 上記ブランチから `feature/confidentiality-classification-sprint3` を切って作業推奨
- **Sprint 2 の状態の再現**: `git log --grep="#188"` で Sprint 2 の全コミットを確認可能

### Sprint 2 で変更されたファイル（Sprint 3 での扱い）
| ファイル | 変更内容 | Sprint 3 での扱い |
|---------|---------|----------------|
| `app/Services/ConfidentialityLevelService.php` | Service 新規作成 | **そのまま流用** |
| `config/confidentiality.php` | 設定ファイル作成 | **そのまま** |
| `database/migrations/2026_05_01_000001_...` | abbreviation カラム追加 | **適用済み** |
| `database/migrations/2026_05_01_000002_...` | 秘密区分カラム追加 | **適用済み** |
| `app/Models/Folder.php` | `$fillable` + casts + `booted()` | **そのまま** |
| `app/Models/LedgerDefine.php` | `$fillable` + casts | **そのまま** |
| `app/Models/Organization.php` | `$fillable` + `booted()` | **そのまま** |
| `app/Models/Role.php` | `$fillable` + `booted()` | **そのまま** |
| `app/Livewire/Folder/FolderForm.php` | TODO コメントを `#189-Sprint3` に更新 | **Service 呼び出しに置き換え** |
| `app/Livewire/LedgerDefine/Edit.php` | TODO コメントを `#189-Sprint3` に更新 | **Service 呼び出しに置き換え** |
| `resources/views/components/ledger/confidentiality-stamp.blade.php` | Sprint 1 で作成済み | **そのまま流用** |
| `resources/views/livewire/folder/folder-form.blade.php` | フォームレイアウト（Sprint 1） | **レイアウトそのまま** |
| `resources/views/livewire/ledger-define/edit.blade.php` | フォームレイアウト（Sprint 1） | **レイアウトそのまま** |
| `resources/views/livewire/ledger/index-manager.blade.php` | スタンプコメントアウト（Sprint 1） | **Sprint 3 で動的データで有効化** |

### Sprint 3 開始時の注意点
1. **Service の呼び出し**: `FolderForm.php` / `Edit.php` の `render()` に `TODO(#189-Sprint3)` が付いた箇所を `ConfidentialityLevelService::selectOptions()` / `allScopes()` に置き換える
2. **保存ロジック**: `save()` / `store()` で `confidentiality_level` と `confidentiality_scopes` をモデルに設定する処理を追加
3. **スタンプの有効化**: `index-manager.blade.php` のコメントアウトを解除し、`ConfidentialityLevelService::getEffectiveLevel()` で動的データを渡す
4. **バリデーション**: LedgerDefineEdit に `confidentialityLevel` のバリデーションを追加

### 未確定事項（Sprint 3 で決定が必要）
- `confidentialityScopes` の保存時 JSON 変換（`"org:1"` → `{"org_ids":[{"id":1}]}`）
- 選択コンポーネント（`x-mary-choices` vs `x-mary-choices-offline`）
- Intersection Observer の実装方法 → ✅ Sprint 3A-4 で完了
- 翻訳キーの追加が必要な箇所の洗い出し → ✅ Sprint 3-5 / 3A で完了
