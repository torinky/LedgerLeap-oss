...
**作成日:** 2026年3月1日  
**更新日:** 2026年3月2日  
**ステータス:** 🚧 Sprint 1〜6 完了 / Sprint 7 計画中  
...
### 🏁 Sprint 7: 台帳リスト画面と同等の行表示（UIテスト指摘 第2弾）
...
- [ ] **Task 7.1.1**: `table-row.blade.php` に `$relatedBadge` スロット prop を追加
  - `@props([... 'relatedBadge' => null])` を追加
  - `updated_at` セルのオーバーレイ `div` 内（スコア・ステータスバッジの手前）に `{{ $relatedBadge }}` を差し込む
  - 既存利用箇所（`records-table.blade.php`）への影響なし（デフォルト null）

#### Block 7.2: `related-ledgers.blade.php` リファクタリング

- [ ] **Task 7.2.1**: 独自テーブル（`<table>` + 独自 `<thead>` + 独自 `<tbody>`）を廃止
- [ ] **Task 7.2.2**: `x-ledger.table-header` + `x-ledger.table-row` の組み合わせに置き換え
  - `x-ledger.table-row` に以下を渡す：
    - `:ledgerRecord="$item['ledger']"`
    - `:allAttachments="$allAttachments"`
    - `:filteredColumnDefines="$columns->toArray()"`
    - `:canUpdate="$canUpdate"` / `:canView="$canView"`
    - `:currentTenantId="$currentTenantId"`
    - `:highlightKeyword="null"`
  - `$relatedBadge` スロットに `x-ledger.related-reason-badge` を渡す：
    - `:reason="$item['reason']"` / `:matchedKeys="$item['matched_keys']"` / `:score="$item['score']"`
- [ ] **Task 7.2.3**: `x-ledger.table-header` を使ってヘッダーを統一（識別理由列ヘッダーは不要）

#### Block 7.3: `show.blade.php` タブバッジ更新

- [ ] **Task 7.3.1**: タブラベルの括弧書き件数を `<span class="badge ...">` バッジ形式に変更
  - `{{ $relatedCount > 0 ? ' (' . $relatedCount . ')' : '' }}` → `x-mary-tab` の `badge` prop またはインライン badge span

#### Block 7.4: テスト整備

- [ ] **Task 7.4.1**: `table-row` の `$relatedBadge` スロット追加に関するリグレッションテスト確認
- [ ] **Task 7.4.2**: `./vendor/bin/sail test` 全テストパス確認
...
- [ ] **Task 7.5.2**: ブラウザ動作確認
  - 添付ファイルが表示される
  - ラベル表示・マークダウン HTML 化が機能する
  - 「もっと見る」が機能する
  - 識別理由バッジが `updated_at` セルオーバーレイに表示される
  - タブのバッジが件数を正しく表示する
- [ ] **Task 7.5.3**: `laravel-boost` エラーチェック
...
| Sprint 7 | 台帳リスト画面と同等の行表示（添付ファイル・標準レンダリング・タブバッジ） | 2〜3時間 |
| **合計** | | **残り Sprint 7 のみ** |
