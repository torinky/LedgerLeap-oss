...
### 6.0. [2025-08-02] `AutoLinkService` の責務変更と関連箇所の修正計画 - <span style="color: green;">完了</span>
...
*   **対応内容と結果:**
    *   **1. `app/Services/AutoLinkService.php` の修正:**
        *   計画通り、`MarkdownRenderer` への依存を完全に削除し、`convert` メソッドがHTML文字列のみを受け付けるように修正した。これにより、サービスの責務が明確になった。

    *   **2. Bladeテンプレートの修正と確認:**
        *   台帳定義の説明文（Markdownで保存）を表示する各Bladeテンプレートについて、`AutoLinkService` を呼び出す前に `app(Spatie\LaravelMarkdown\MarkdownRenderer::class)->toHtml(...)` を使ってHTMLに変換するよう修正した。
        *   **当初の計画からの変更点:**
            *   調査の過程で `resources/views/ledger/show.blade.php` での変換漏れが発覚したため、これを修正した。
            *   追加で `resources/views/ledger/edit.blade.php` も確認し、こちらは正しく実装されていることを確認した。
        *   **最終的な対象ファイル:**
...
*   **結論:**
    *   `AutoLinkService` の責務分離は完了し、関連する全てのビューで、Markdownコンテンツが正しくHTMLに変換された後で自動リンク処理が行われるようになった。これにより、当初の課題は解決された。
