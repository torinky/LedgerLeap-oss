...
*   **ファイルダウンロード API**:
    *   機能: 添付ファイルのダウンロード。
    *   パス: `/files/{attachedFile}/download`
    *   概要: 認可チェック（ユーザーがそのファイルを含む台帳を閲覧できるか）とダウンロードログの記録を行った上で、安全にファイルを配信します。`?thumbnail=true` パラメータでサムネイル画像を、`?original=true` パラメータでOCR処理前のオリジナルファイルを取得することも可能です。
*   **ユーザー管理 API**:
