...
* **表現力豊かなテキスト入力:** 複数行の入力項目ではMarkdown記法をサポートし、業務日報や申し送り事項などを構造的に記述できます。
...
* **自動リンク機能**: 台帳の本文や台帳定義の説明文に含まれる特定の文字列（例: `SPEC-001`）を、仕様書や外部システムのチケットなど、関連情報へ自動的にハイパーリンクを生成します。
