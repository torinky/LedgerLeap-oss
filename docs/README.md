...
* **強力な全文検索とOCR/VLM**: 
    ...
    * **OCR機能**: **OcrMyPDF** を利用し、画像ファイルやスキャンされたPDFからテキストを抽出。
    * **VLM機能** (✅ Phase4実装完了): **PaddleOCR-VL** による高精度なビジュアル言語モデルOCR。手書き文字や複雑なレイアウトにも対応し、OCR失敗時の自動フォールバックにより検索能力を大幅に向上。
* **非同期処理による堅牢なファイル解析**: 
...
* **VLM (Visual Language Model)**: 画像やPDFからテキストを抽出する高精度なAIモデル。PaddleOCR-VL 0.9Bを採用し、手書き文字認識やMarkdown生成が可能。OCR失敗時の自動フォールバックにより、検索精度を大幅に向上。
    ...
    * **作業ファイル（計画・設計・作業ログ）:**
        * `/work/`: 実装計画、設計書、作業記録
            * `/llm-integration/`: LLM連携機能の計画・実装記録
                * **主要文書:** [開発ロードマップ](./work/llm-integration/2025-09-23_LLM_Integration_Roadmap.md), [API仕様書](./work/llm-integration/2025-09-24_LLM_Phase1_API_Specification.md)
                * **実装計画との対応:** 作業ファイルの実装結果は公式ドキュメント（特に `/development/MCP_Architecture_and_Flow.md`）に反映されています
* `/lang`: 言語ファイル
...
2. **作業ファイル** (`/docs/work/`)
   ...
   - 例: [LLM連携機能 開発ロードマップ](./work/llm-integration/2025-09-23_LLM_Integration_Roadmap.md)
