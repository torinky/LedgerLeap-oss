<?php

return [
    'Impersonate' => 'なりすまし',
    'Impersonation' => 'なりすまし',
    'action_approval_requested' => 'の承認を依頼しました',
    'action_approved' => 'を承認しました',
    'action_attached' => 'を割り当てました',
    'action_created' => 'を作成しました。',
    'action_deleted' => 'を削除しました。',
    'action_detached' => 'の割り当てを解除しました',
    'action_edited_while_pending' => 'を編集しました(要再申請)',
    'action_error' => 'アクションの実行に失敗しました:',
    'action_inspection_completed' => 'の点検を完了しました',
    'action_inspection_requested' => 'の点検を依頼しました',
    'action_login' => 'ログインしました',
    'action_logout' => 'ログアウトしました',
    'action_returned_to_draft' => 'を作成中に戻しました',
    'action_success' => 'アクションが正常に実行されました。',
    'action_task_claimed' => 'を引き継ぎました',
    'action_updated' => 'を更新しました。',
    'ad_last_synced_at' => 'AD最終同期日時',
    'ad_sync_status_title' => 'AD連携ステータス',
    'add' => '追加する',
    'admin' => '管理',
    'admin_email' => 'システム管理者からのメール',
    'admin_announcement_banner_title' => 'システム管理者からのお知らせ',
    'admin_announcement_banner_body_hint' => '短く要点をまとめると見やすくなります。',
    'admin_announcement_banner_cta_hint' => '任意。詳細ページへ誘導するリンクです。',
    'admin_announcement_banner_cta_label' => 'リンクテキスト',
    'admin_announcement_banner_cta_title' => 'リンク',
    'admin_announcement_banner_cta_url' => 'リンク先URL',
    'admin_announcement_banner_default_body' => '管理者向けのお知らせ本文を入力してください。',
    'admin_announcement_banner_default_title' => '管理者からのお知らせ',
    'admin_announcement_banner_description' => 'Filament のトップ画面から編集できる管理者向けのお知らせです。',
    'admin_announcement_banner_field_title' => '見出し',
    'admin_announcement_banner_form_hint' => '公開前の内容をこの画面で整えます。',
    'admin_announcement_banner_form_title' => '入力フォーム',
    'admin_announcement_banner_creator_label' => '作成者',
    'admin_announcement_banner_list_current_title' => '編集中',
    'admin_announcement_banner_list_empty' => 'お知らせはありません。',
    'admin_announcement_banner_list_published_title' => '公開済み',
    'admin_announcement_banner_ends_at' => '終了日時',
    'admin_announcement_banner_management_hint' => '編集中の内容と公開済みのお知らせを一覧で確認できます。',
    'admin_announcement_banner_management_title' => '管理一覧',
    'admin_announcement_banner_level_label' => 'レベル',
    'admin_announcement_banner_modifier_label' => '更新者',
    'admin_announcement_banner_display_state_title' => 'バナーの提示範囲と状態',
    'admin_announcement_banner_page_title' => 'システム管理者からのお知らせ',
    'admin_announcement_banner_preview_hint' => '右側のプレビューは、入力内容の見え方を確認するためのものです。',
    'admin_announcement_banner_preview_reset' => 'プレビューをリセット',
    'admin_announcement_banner_preview_summary' => 'システム管理者に関するお知らせです',
    'admin_announcement_banner_preview_title' => '表示プレビュー',
    'admin_announcement_banner_publish_scope' => '公開範囲',
    'admin_announcement_banner_scope_hint' => '表示対象を複数選択できます。',
    'admin_announcement_banner_publish_action' => '公開する',
    'admin_announcement_banner_reset' => '初期値に戻す',
    'admin_announcement_banner_archive_action' => '停止する',
    'admin_announcement_banner_scope_all_tenants' => '全テナント',
    'admin_announcement_banner_scope_current_tenant' => '現在のテナント',
    'admin_announcement_banner_status_archived' => '停止中',
    'admin_announcement_banner_status_draft' => '下書き',
    'admin_announcement_banner_status_hint' => '公開状態は上部の操作ボタンで切り替え、日時は公開期間として管理します。',
    'admin_announcement_banner_status_label' => '公開状態',
    'admin_announcement_banner_status_ended' => '公開終了',
    'admin_announcement_banner_status_published' => '公開中',
    'admin_announcement_banner_status_scheduled' => '公開予定',
    'admin_announcement_banner_save_draft_action' => '下書き保存',
    'admin_announcement_banner_sticky_helper' => '固定表示にすると、重要なお知らせを常に見せられます。重大レベルでは自動的に固定表示になります。',
    'admin_announcement_banner_starts_at' => '開始日時',
    'admin_announcement_banner_sticky_label' => '固定表示',
    'admin_announcement_banner_sticky_off' => '固定表示しない',
    'admin_announcement_banner_sticky_on' => '固定表示する',
    'admin_announcement_banner_timing_hint' => '公開の開始・終了を指定できます。',
    'admin_announcement_banner_timing_title' => '公開期間',
    'admin_announcement_banner_archived' => '停止状態にしました。',
    'admin_announcement_banner_published' => '公開状態にしました。',
    'admin_announcement_banner_validation_body_required' => '本文を入力してください。',
    'admin_announcement_banner_validation_cta_url' => 'リンク先URLの形式が正しくありません。',
    'admin_announcement_banner_validation_ends_at_after_or_equal' => '終了日時は開始日時以降にしてください。',
    'admin_announcement_banner_validation_ends_at_required' => '終了日時を入力してください。',
    'admin_announcement_banner_validation_level_required' => 'レベルを選んでください。',
    'admin_announcement_banner_validation_scope_required' => '公開範囲を選んでください。',
    'admin_announcement_banner_validation_starts_at_required' => '開始日時を入力してください。',
    'admin_announcement_banner_validation_status_required' => '公開状態を選んでください。',
    'admin_announcement_banner_validation_title_required' => '見出しを入力してください。',
    'after_change' => '変更後',
    'after_change_colon' => '変更後:',
    'agree' => '同意する',
    'all' => 'すべて',
    'all_accessible_folders_link' => 'アクセス可能な全フォルダと権限を確認する',
    'all_operations' => '全ての操作',
    'all_organizations' => '全ての組織',
    'all_permissions' => '全ての権限',
    'all_roles' => '全てのロール',
    'all_users' => '全てのユーザー',
    'archive' => 'アーカイブ',
    'ascending' => '昇順',
    'assign' => '割当てる',
    'assigned_folders_title' => 'あなたが担当する主なフォルダ',
    'associate' => '仲間',
    'attach' => '添付する',
    'attach_existing_folder' => '既存のフォルダーを紐付ける',
    'attach_existing_organization' => '既存組織を紐付け',
    'attribute' => '属性',
    'auto_sync_enabled' => '自動同期中',
    'back' => '戻る',
    'back_to_list' => 'リストに戻る',
    'basic_information' => '基本情報',
    'breadcrumb_top' => 'トップ',
    'modified_by' => '最終変更者',
    'basic_operations_info' => 'その他、フォルダの権限に応じて台帳の閲覧などの基本的な操作が可能です。',
    'basic_operations_permission' => '台帳の閲覧などの基本的な操作が可能です。',
    'before_change' => '変更前',
    'before_change_colon' => '変更前:',
    'browse' => 'ブラウズ',
    'cancel' => 'キャンセル',
    'cancel_and_close' => 'ウィンドウを閉じる',
    'central_settings' => 'システム設定',
    'changed' => '変更あり',
    'changes' => '変更点',
    'choose' => '選ぶ',
    'choose_file' => 'ファイルを選択',
    'choose_image' => '画像を選択',
    'click_to_copy' => 'クリックしてコピー',
    'close' => '閉じる',
    'close_view_window_message' => '閲覧を終了してウィンドウを閉じますか？',
    'close_window' => 'ウィンドウを閉じる',
    'close_window_message' => '編集内容を破棄してウィンドウを閉じますか？',
    'close_window_modal' => '閉じようとしています',
    'collapse' => '折りたたむ',
    'collapse_all' => 'すべて折りたたむ',
    'column_deleted' => 'カラム削除 (:id)',
    'column_saved' => 'カラムを保存しました。',
    'comment' => 'コメントする',
    'composite_score_tooltip' => '総合スコア（活動度・新鮮度・ステータス）',
    'confirm' => '確認する',
    'connect' => '接続する',
    'containing' => '含む',
    'content' => '内容',
    'continue_edit' => '編集を続ける',
    'continue_editing' => '編集を続ける',
    'create' => '新規追加',
    'create_message' => '新規登録する',
    'create_title' => '台帳に新規追加',
    'created_at' => '作成日',
    'info' => '情報',
    'warning' => '警告',
    'critical' => '重大',
    'published_at' => '発信日時',
    'current_tenant' => '現在のテナント',
    'default_sort_order' => 'デフォルト順',
    'delete' => '削除する',
    'delete_confirm' => '削除する',
    'descending' => '降順',
    'description' => '説明',
    'detach' => '切り離す',
    'detach_confirmation_description' => 'この組織を現在の配下から解除します。組織自体は削除されません。よろしいですか？',
    'detach_confirmation_title' => '紐付け解除の確認',
    'detailed_information_title' => '詳細情報',
    'details' => '詳細',
    'disable' => '無効にする',
    'disable_notification' => '通知を無効にする',
    'disable_notification_confirm' => 'このフォルダーの通知設定を削除しますか？',
    'discard' => '破棄する',
    'done' => '完了',
    'down' => '下へ',
    'download' => 'ダウンロード',
    'download_optimized' => '最適化PDFをダウンロード',
    'download_original' => 'オリジナルをダウンロード',
    'draft_saved' => '下書きを保存しました。',
    'duplicate' => '重複',
    'duplicate_from_this' => 'この内容で新規作成',
    'duplicated_from' => 'コピー元',
    'edit' => '編集する',
    'editTitle' => '台帳の編集',
    'edit_notifications_settings_title' => '通知される条件を変更する',
    'edit_profile_title' => 'プロフィールを変更する',
    'empty' => 'データなし',
    'enable' => '有効にする',
    'expand' => '拡大する',
    'expand_all' => 'すべて展開',
    'explanation' => '説明',
    'export' => 'エクスポート',
    'export_csv' => 'CSVをエクスポート',
    'export_finished' => 'エクスポート完了',
    'exporting' => 'エクスポート中です。お待ちください・・・',
    'export_ready' => 'CSVの準備が完了しました。ダウンロードできます。',
    'file' => 'ファイル',
    'file_list' => '添付ファイル一覧',
    'file_upload_failed' => 'ファイルアップロード失敗: :filename',
    'files' => 'ファイル',
    'filter' => 'フィルタ',
    'filter_expired_users' => '期限切れユーザーをフィルタ',
    'folder_permission_editable' => '編集可能',
    'folder_permission_manageable' => '管理可能',
    'folder_permission_readable' => '閲覧可能',
    'folders' => 'フォルダー',
    'folders_to_attach_under' => '配下に紐付けるフォルダー',
    'guideline' => 'ガイドライン',
    'go_home' => '設定を終了',
    'go_to' => 'この台帳のリスト表示へ移動',
    'go_to_folder_button' => 'フォルダへ移動',
    'has_been_created' => '作成されました',
    'has_been_updated' => '更新されました',
    'hide' => '隠す',
    'home' => 'home',
    'ignore_ad_org_sync_until' => '組織手動管理期限',
    'image' => '画像',
    'impersonate' => 'なりすます',
    'impersonation' => 'なりすまし',
    'import' => 'インポート',
    'import_file' => 'ファイルからインポート',
    'introduction' => '導入',
    'landing_page_option_ledgers' => '台帳/フォルダ一覧',
    'landing_page_option_my_portal' => 'マイポータル',
    'landing_page_select_label' => '初期画面',
    'landing_page_setting_description' => 'ログイン後に最初に表示する画面を選択してください。',
    'landing_page_setting_title' => 'ログイン後の初期画面設定',
    'last_synced_at' => '最終同期日時',
    'ledger' => '台帳',
    'ledger_define' => '台帳定義',
    'ledger_define_id' => '台帳定義ID',
    'like' => 'のように',
    'link_unavailable' => 'リンクなし',
    'load' => '負荷',
    'loading' => '読み込み中...',
    'localize' => 'ローカライズ',
    'log_in' => 'ログイン',
    'log_out' => 'ログアウト',
    'main_abilities_title' => '主なできること',
    'portal_primary_organization_label' => '主所属',
    'portal_primary_role_desc' => 'このポータルで見ている代表ロール',
    'portal_primary_role_label' => '主なロール',
    'portal_roles_subtitle' => '所属と役割を先に確認できます。',
    'main_role_and_affiliation_title' => '所属と役割の概要',
    'manual_sync_enabled' => '手動管理中',
    'manual_sync_expired_warning_body' => '期限切れの手動管理ユーザーが:count名います。早急に対応してください。',
    'manual_sync_expired_warning_title' => '期限切れの手動管理ユーザー',
    'manual_sync_reason' => '手動管理理由',
    'manual_sync_status_label' => '組織手動管理ステータス',
    'manual_sync_until' => '(期限: :date)',
    'mark_all_as_read' => 'すべて既読にする',
    'mark_as_read' => '既読にする',
    'message' => 'お知らせ',
    'modifies' => '変更履歴を確認',
    'modify_message' => '変更する',
    'more_files' => ':count件の追加ファイル',
    'my_portal_title' => 'マイポータル',
    'portal_notifications_subtitle' => 'システムとワークフローの通知をまとめて確認できます。',
    'portal_notifications_summary' => '未読とシステムのお知らせをまとめて確認できます。',
    'portal_folder_handoff_title' => 'フォルダから台帳一覧へ',
    'portal_folder_tree_hint' => 'フォルダ名をクリックすると、そのフォルダの台帳一覧へ移動します。',
    'name' => '名前',
    'new' => '新着',
    'new_relation_attach' => '紐付ける',
    'next' => '次へ',
    'no' => 'いいえ',
    'no_activity_logs' => 'アクティビティログはありません。',
    'no_assigned_folders' => '担当フォルダはありません。',
    'no_attachments' => '添付ファイルはありません',
    'no_comment' => '理由の記載はありません',
    'no_content_in_this_diff' => 'この履歴には内容の変更記録はありません。',
    'no_create_permission' => '新規作成の権限がありません',
    'no_edit_permission' => '編集権限がありません',
    'no_manage_permission' => '管理権限がありません',
    'no_manage_permissions' => '管理権限がありません',
    'no_notification' => '通知はありません',
    'no_organization_assigned' => '組織に所属していません',
    'no_primary_organization' => '主所属なし',
    'no_roles_assigned' => 'ロールが割り当てられていません。',
    'no_specific_role' => '特定のロールなし',
    'no_synonyms' => '同義語なし',
    'no_view_permission' => '閲覧権限がありません',
    'no_view_permissions' => '閲覧権限がありません',
    'none' => 'なし',
    'not_allow_create' => '台帳を作成する権限がありません',
    'not_allow_delete' => '削除権限がありません',
    'not_allow_destroy' => '削除権限がありません',
    'not_allow_manage' => '管理権限がありません',
    'not_allow_view' => '閲覧権限がありません',
    'not_found' => '見つかりません',
    'notification_type' => '通知タイプ',
    'notifications' => '通知',
    'notify' => '通知',
    'of' => 'の',
    'open' => '開ける',
    'open_tree_view' => 'ツリー表示',
    'open_website' => 'ウェブサイトで開く',
    'opened_count' => '検索条件にヒットした件数',
    'optimized_pdf' => '最適化PDF',
    'options' => '選択肢',
    'organization' => '組織',
    'organization_abbreviation' => '略称',
    'organization_abbreviation_placeholder' => '正式名を省略した表記（例：総務部）',
    'organization_empty' => '所属している組織やプロジェクトはありません。',
    'organization_section_description' => 'あなたが所属している組織やプロジェクトの一覧です。',
    'organization_section_title' => '所属組織/プロジェクト',
    'organizations_to_attach_under' => '配下に紐付ける組織',
    'original_file' => 'オリジナル',
    'other_affiliations_title' => 'その他の所属',
    'per_page' => '表示件数',
    'performed_action' => 'さんが ',
    'preview' => 'プレビュー',
    'price' => '価格',
    'processing_completed_with_action' => '処理が完了しました: :action',
    'record' => '記録',
    'records' => '件',
    'records_title' => 'リスト',
    'remove' => '削除する',
    'remove_message' => 'この案件の内容は削除されます！ 取り消しできません！',
    'remove_records_message' => 'この台帳のレコードを削除します',
    'remove_success' => '削除しました',
    'remove_title' => '削除しようとしています',
    'reset' => 'リセット',
    'reset_search' => '検索条件をリセット',
    'restore' => '復元する',
    'reviseCount' => '改訂回数',
    'role' => 'ロール',
    'roles_and_affiliations_title' => 'ロールと所属',
    'save' => '保存する',
    'save_and_close' => '保存して閉じる',
    'save_and_return' => '保存して戻る',
    'save_and_return_to_draft' => '保存して作成中に戻す',
    'save_changes' => '上書きする',
    'save_draft' => '下書き保存',
    'search' => '検索する',
    'search_active' => '検索語あり',
    'search_hero_description' => 'キーワード、同義語、技術用語で素早く絞り込めます',
    'search_hero_hint' => 'スペース区切りで複数語を検索できます',
    'search_hero_title' => '台帳を検索',
    'search_keyword' => '検索キーワード',
    'search_message' => 'スペース区切りでキーワードを検索',
    'search_suggest' => [
        'recent' => '最近の検索',
        'saved' => '保存した検索',
        'popular' => 'よく検索されている',
        'popular_fallback' => 'よく検索されているキーワード',
        'related' => '関連する検索',
        'items' => '件',
        'save' => 'この条件を保存',
        'save_name' => '保存名',
        'delete' => '削除',
        'empty_label' => '検索条件',
        'cannot_restore' => 'この検索条件は現在の権限では復元できません。',
    ],
    'search_tag_active' => 'タグでフィルタ中',
    'search_options' => '検索オプション',
    'search_synonym' => '同義語も検索',
    'search_synonym_hint' => '同義語を候補に含めます',
    'search_technical_term' => '技術用語も検索',
    'search_technical_term_hint' => '技術用語も含めて検索します',
    'search_view' => '台帳リスト',
    'searched' => '検索しました',
    'select' => '選択する',
    'go_to_ledger_list_button' => 'このフォルダの台帳一覧へ移動',
    'select_all' => 'すべて選択',
    'select_folders_to_attach' => '紐付けるフォルダーを選択',
    'select_message' => 'フォルダまたは台帳を選択',
    'select_organizations_to_attach' => '紐付ける組織を選択...',
    'select_roles' => 'ロールを選択',
    'semantic_score_sort' => '意味検索スコア順',
    'semantic_score_tooltip' => '検索語との意味的な関連度（AI判定）',
    'semantic_search' => '意味検索',
    'semantic_search_active' => '意味検索モードで検索中',
    'semantic_search_requires_query' => '意味検索を使用するには検索語を入力してください',
    'send' => '送信する',
    'setting' => '設定',
    'show' => '見せる',
    'show_all' => 'すべて表示する',
    'show_details' => '詳細を表示する',
    'show_diff' => '変更内容を表示する',
    'show_diff_hint' => 'ONにすると、前回保存時から変更された項目がハイライトされます。',
    'show_less' => '少なく表示',
    'show_more' => 'もっと見る',
    'sign_in' => 'サインイン',
    'solve' => '解決する',
    'sort_by' => '並び順',
    'sort_direction' => '昇順 / 降順',
    'sort_priority' => '優先度',
    'start' => '始める',
    'stop' => '停止する',
    'submit' => '提出する',
    'subscribe' => '購読する',
    'success' => '成功',
    'switch' => 'スイッチ',
    'switch_to_role' => 'ロールに切り替える',
    'synonym_disabled_in_semantic_search' => '意味検索では同義語検索は使用されません',
    'system_name' => 'Ledger Leap',
    'tags' => 'タグ',
    'technical_term' => '技術用語',
    'tenant' => 'テナント',
    'tenant_context_notification_body' => 'テナント「:name」のコンテキストで中央管理画面を表示しています。',
    'tenant_context_notification_title' => '管理画面のコンテキスト表示',
    'tenant_creation_failed' => 'テナントの初期設定に失敗しました',
    'tenant_creation_successful' => 'テナントの初期設定が完了しました',
    'tenant_id' => 'テナントID',
    'tenant_name' => 'テナント名',
    'tenant_settings' => 'テナント設定',
    'title' => '台帳名',
    'title_unknown' => 'タイトル不明',
    'total' => '合計',
    'translate' => '翻訳する',
    'translate_it' => '翻訳してください',
    'type_here' => 'ここに入力',
    'unassigned' => '未割当',
    'unknown' => '不明',
    'unknown_ledger' => '不明な台帳',
    'unknown_user' => '不明なユーザー',
    'unpack' => '展開する',
    'unread' => '未読',
    'unsubscribe' => '購読を解除する',
    'up' => '上',
    'update' => 'アップデート',
    'update_message' => '更新する',
    'updated_at' => '更新日時',
    'last_updated_at' => '最終更新日時',
    'user' => 'ユーザー',
    'user_action_suffix' => ' が ',
    'user_object_suffix' => ' を ',
    'version' => 'バージョン',
    'view' => 'ビュー',
    'view_content_at_this_point' => 'この時点の内容を見る',
    'view_details' => '詳細確認',
    'view_history' => '変更履歴を見る',
    'yes' => 'はい',
    'your_effective_roles_title' => 'あなたの有効なロール',
    'action_bar_open' => 'アクションを開く',
    'action_bar_close' => 'アクションを閉じる',
    'export_csv_download' => 'CSVをダウンロード',

    'footer' => [
        'contact' => '連絡先',
        'support' => 'サポート',
        'support_email' => 'メールで問い合わせ',
        'forum' => 'フォーラム',
        'all_rights_reserved' => 'All rights reserved.',
        'version' => 'バージョン',
    ],
];
